...
export default function AdminAccountsPage() {
  const [items, setItems] = React.useState<AccountItem[] | null>(null);
  const [reason, setReason] = React.useState<string | null>(null);
  React.useEffect(() => {
    const controller = new AbortController();
    void fetch("/api/trader/admin/console/accounts", { signal: controller.signal })
      .then(
        async (response) =>
          response.json() as Promise<{ data?: { items?: AccountItem[]; reasons?: string[] } }>,
      )
      .then((body) => {
  ...
  return (
    <section className="grid gap-3">
      <h2 className="text-xl font-semibold">{RU.sections.accounts}</h2>
      {reason ? <DataState state="unavailable" reason={reason} /> : null}
      {items ? (
        <ul>
          {items.map((item) => (
            <li key={item.id}>
              {`${item.venue} ${item.exchangeAccountId}${item.revokedAt ? " — отозван" : ""}`}
            </li>
          ))}
        </ul>
      ) : null}
