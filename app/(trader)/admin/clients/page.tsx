        ...
        >
          <ConsoleTable
            ...
            columns={[
              ...
              { title: "Подключён с", render: (row) => row.connectedSince },
              {
                title: "Организация",
                render: (row) => (
          ...
          <div className="space-y-5">
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div>
                ...
                <p className="mt-2 text-sm">{selected.connectedSince}</p>
              </div>
              <div>
                <p className="text-waia-fg-muted text-xs">Владелец</p>
                <p className="mt-2 text-sm break-words">{selected.ownerEmail}</p>
              </div>
            </div>
            <div className="grid gap-2 sm:grid-cols-2">
              {[
                ["/admin?tab=summary", "Сводка и результаты"],
                ["/admin/accounts", "Биржевые счета"],
                ["/admin/clients?tab=invoices", "Счета на оплату"],
                ["/admin/clients?tab=payments", "Платежи"],
                ["/admin/clients?tab=periods", "Отчётные периоды"],
                ["/admin/accounts?tab=history", "История подключений"],
              ].map(([path, label]) => (
                <Link
                  key={path}
                  ...
                  className="border-waia-rim hover:bg-waia-elevated rounded-lg border px-4 py-3 text-sm"
                >
                  {label} ↗
                </Link>
              ))}
