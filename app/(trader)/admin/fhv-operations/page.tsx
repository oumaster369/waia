...
export default function FhvOperationsAdminPage(){
  ...
  const requestedOrganization=params.get("organization_id")?.trim()??"";
  const preferredOrganization=selected||requestedOrganization;
  const organizationId=preferredOrganization
    ? organizations.find(org=>org.id===preferredOrganization)?.id??""
    : organizations[0]?.id??"";
