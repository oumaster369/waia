...
import { createStrategyTrialServicePostgres } from "@/lib/trader/intelligence/strategies/strategy-trial-service";
...
describe.skipIf(!integrationEnabled || !url)(
  "postgres trader strategy trial parity (DEE-415 / HTR-WP16)",
  () => {
    let orgA: string;

    beforeAll(async () => {
      await cleanupWp16Org(url!, WP16_PG_USER_A);
      orgA = await seedWp16User(url!, WP16_PG_USER_A, "WP16 Trial Parity");
    });
    ...
    it("persists append-only trial events and derived counts", async () => {
      const db = getPostgresDrizzle();
      const service = createStrategyTrialServicePostgres(db);
      const context = { organizationId: orgA };
      const base = {
        strategyId: "mean_reversion_v0",
        strategyVersion: "0.1.0",
        runId: "wp16-trial-run",
        cycleId: "0",
        symbol: "BTC/USDT",
        accountKey: "acct",
        ...
        registeredBy: "wp16-test",
      };
      await service.registerStrategyTrial(context, {
        ...base,
        deterministicId: "00000000-0000-4000-8000-000000000201",
      });
      await service.registerStrategyTrial(context, {
        ...base,
        cycleId: "1",
        deterministicId: "00000000-0000-4000-8000-000000000202",
      });

      const rows = await db
        .select()
        .from(pgSchema.traderStrategyTrial)
        .where(eq(pgSchema.traderStrategyTrial.organizationId, orgA));
