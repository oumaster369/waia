...
import { createAccountDrawdownRepositoryPostgres } from "@/lib/trader/risk/account-drawdown-repository-postgres";
...
describe.skipIf(!integrationEnabled || !url)(
  "postgres trader account drawdown parity (DEE-415 / HTR-WP16)",
  () => {
    let orgA: string;

    beforeAll(async () => {
      await cleanupWp16Org(url!, WP16_PG_USER_A);
      orgA = await seedWp16User(url!, WP16_PG_USER_A, "WP16 Account Drawdown Parity");
    });
    ...
    it("persists append-only account drawdown checkpoints", async () => {
      const db = getPostgresDrizzle();
      const repo = createAccountDrawdownRepositoryPostgres(db);
      ...
      const rows = await db
        ...
        .where(eq(pgSchema.traderAccountDrawdownCheckpoint.organizationId, orgA));
      ...
      expect(latest?.accountPeakHwm).toBe("100000");
    });
  },
);
