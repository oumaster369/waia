...
import { createStrategyLifecycleServicePostgres } from "@/lib/trader/intelligence/strategies/strategy-lifecycle-service";
...
describe.skipIf(!integrationEnabled || !url)(
  "postgres trader strategy lifecycle parity (DEE-415 / HTR-WP16)",
  () => {
    let orgA: string;

    beforeAll(async () => {
      await cleanupWp16Org(url!, WP16_PG_USER_A);
      orgA = await seedWp16User(url!, WP16_PG_USER_A, "WP16 Lifecycle Parity");
    });
    ...
    it("persists append-only lifecycle events with as-of parity", async () => {
      const db = getPostgresDrizzle();
      const service = createStrategyLifecycleServicePostgres(db);
      const context = { organizationId: orgA };
      await service.appendLifecycleEvent(context, {
        id: "00000000-0000-4000-8000-000000000101",
        strategyId: "mean_reversion_v0",
        strategyVersion: "0.1.0",
        transition: { fromState: null, toState: "DRAFT", actor: "MACHINE" },
      ...
      await service.appendLifecycleEvent(context, {
        ...
        strategyId: "mean_reversion_v0",
        strategyVersion: "0.1.0",
        transition: { fromState: "DRAFT", toState: "RESEARCHING", actor: "MACHINE" },
      ...
      const rows = await db
        ...
        .where(eq(pgSchema.traderStrategyLifecycleEvent.organizationId, orgA));
