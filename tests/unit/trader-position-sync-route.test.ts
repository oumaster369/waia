...
import { POST as syncPositionsRoutePost } from "@/app/api/trader/exchange-credentials/[credentialId]/sync-positions/route";
import { GET as positionSnapshotsGet } from "@/app/api/trader/position-snapshots/route";
...
import { HTX_POSITION_SYNC_ERROR_CODES } from "@/lib/trader/positions/sync-api.types";
import {
...
const USER_WITH_TRADER = "00000000-0000-4000-8000-000000003501";
const USER_NO_TRADER = "00000000-0000-4000-8000-000000003502";
...
describe("HTX position sync API (DEE-350)", () => {
  ...
  beforeAll(async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "waia-htx-position-sync-"));
    process.env.DATABASE_URL = `file:${path.join(tmpDir, "htx-position-sync.sqlite")}`;
    migrateDatabaseFromEnv();
    const db = getDb();

    insertEmailPasswordUser(db, {
      id: USER_WITH_TRADER,
      email: "htx-position-trader@waia.invalid",
      password: "password123",
      identityLabel: "HTX Position Trader",
    });
    insertEmailPasswordUser(db, {
      id: USER_NO_TRADER,
      email: "htx-position-no-trader@waia.invalid",
      password: "password123",
  ...
  function createDeps(overrides: Partial<PositionSyncHandlerDeps> = {}): PositionSyncHandlerDeps {
    ...
    return {
      ...
      createPositionSnapshotService: (runtime) => {
        if (runtime.kind === "sqlite") {
  ...
  it("returns 401 when session is missing (handler)", async () => {
    const result = await handlePositionSyncPost(
      credentialId,
      createDeps({ getUserId: async () => null }),
    );
    expect(result.status).toBe(401);
    expect(result.body).toEqual({
      error: { code: HTX_POSITION_SYNC_ERROR_CODES.UNAUTHORIZED, message: "Session required." },
    });
  });

  it("returns 401 when session is missing (route)", async () => {
    vi.mocked(sessionUser.getOptionalSessionUserId).mockResolvedValue(null);
    const response = await syncPositionsRoutePost(new Request("http://localhost/sync"), {
  ...
  it("returns 403 when trader entitlement is missing", async () => {
    ...
    expect(result.body).toEqual({
      error: {
        code: HTX_POSITION_SYNC_ERROR_CODES.FORBIDDEN,
  ...
  it.each([false, true])("rejects different stored account before data fetch/write (stub=%s)", async (stubValidation) => {
    ...
    const result = await handlePositionSyncPost(
      credentialId,
      createDeps({
        createPositionSnapshotService: () => ({ recordSnapshot }) as never,
        createConnector: (config) => {
          ...
          vi.spyOn(connector, "getPositions").mockImplementation(read);
          if (stubValidation) {
  ...
  it("preserves generic validation failure when no reason is supplied", async () => {
    ...
    const result = await handlePositionSyncPost(credentialId,  createDeps({ createConnector: () => connector }));
    expect(result.status).toBe(502);
  ...
  it("returns 502 when HTX position fetch fails", async () => {
    const mockFetch = defaultHtxHandlers({
      [`/accounts/${SPOT_ACCOUNT_ID}/balance`]: () =>
  ...
  it("syncs positions and returns snapshot without secrets", async () => {
    const result = await handlePositionSyncPost(credentialId, createDeps());
    ...
    expect(positions[0]?.marketType).toBe("spot");
    ...
    const audits = db
      .select()
      .from(auditLogs)
      .where(eq(auditLogs.action, traderAuditActions.positionSnapshotCreated))
      .all();
    expect(audits.some((row) => row.entityId === body.id)).toBe(true);
  });

  it("GET lists snapshots latest-first with limit", async () => {
    await handlePositionSyncPost(credentialId, createDeps());

    const listResult = await handlePositionSnapshotsGet(
      new Request(
        `http://localhost/api/trader/position-snapshots?credentialId=${credentialId}&limit=5`,
      ),
      createDeps(),
    );
  ...
  it("GET route returns 401 without session", async () => {
    vi.mocked(sessionUser.getOptionalSessionUserId).mockResolvedValue(null);
    const response = await positionSnapshotsGet(
      new Request("http://localhost/api/trader/position-snapshots"),
    );
    expect(response.status).toBe(401);
  });
});
