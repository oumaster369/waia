...
describe("trader wp13 conviction record", () => {
  it("uses scope ACTIVE_HYPOTHESIS or NONE only", () => {
    ...
    expect(["ACTIVE_HYPOTHESIS", "NONE"]).toContain(bundle.conviction.convictionScope);
    expect(bundle.conviction.convictionScope === "ACTIVE_HYPOTHESIS").toBe(
      bundle.conviction.activeHypothesisRecordId !== null,
    );
  });
});
