...
} from "@/lib/trader/research/benchmark/cdf-evidence-protocol-v2";
...
describe("DEE-993 current evidence cannot admit legacy/mixed Cody results", () => {
  ...
  beforeAll(() => {
    const input = {
      ...
      comparisonFamilyId: "synthetic-cody-binding",
      evaluationPartitionReceiptDigestHex: hex("6"),
      ...
      anchors: [
        {
          anchorId: "synthetic",
          observedReturn: 1,
          challengerProbabilities: [0, 0, 0, 0, 0, 0, 1],
        },
      ],
    };
  ...
  it.each([true, false])(
    "rejects resealed absent/legacy/wrong protocol before verdict; qualified=%s",
    (qualified) => {
      for (const patch of [
        { cdfKernelVersion: undefined },
        { cdfAmendmentDigestHex: undefined },
        { cdfKernelVersion: "cdf-erf-cody715/v1" },
        { cdfAmendmentDigestHex: hex("f") },
      ]) {
        const tampered = seal({
          ...structuredClone(qualified ? yes : no),
          ...patch,
        } as PredictiveTerminalReceiptV1);
        expect(() => validatePredictiveTerminalReceipt(tampered)).toThrow(
          "SCIENTIFIC_ADMISSION_CDF_CONTRACT_MISMATCH",
        );
      }
    },
  );
  it.each(["mandatory", "availability", "means", "comparisons", "holm", "diagnostics"])(
    "rejects mixed legacy IDs in %s",
    (location) => {
      for (const original of [yes, no]) {
        const r = structuredClone(original),
          old = "gaussian-pop-std/v1";
        ...
        if (location === "comparisons")
          r.holmComparisons = [...r.holmComparisons, { comparisonId: old, pValue: 0.0001 }];
        if (location === "holm")
          r.holmResults = [...r.holmResults, { ...yes.holmResults[0]!, comparisonId: old }];
        if (location === "diagnostics")
          r.logScoreDiagnostics[old] = yes.logScoreDiagnostics["gaussian-pop-std/v2"]!;
  ...
  it("binds changed baseline IDs into trial digests without rewriting trial serialization", () => {
    const base: TrialIdentityInput = {
      scoringContractVersion: identities.scoringContractVersion,
      evaluationPartitionReceiptDigestHex: hex("6"),
      venue: "htx",
      market: "spot",
      symbol: "BTCUSDT",
      primaryHorizonMinutes: 30,
      modelTransformVersion: "empirical/v1",
      challengerPackageContentDigestHex: hex("4"),
      baselineId: "unused",
      metricId: "terminal-multiclass-brier-reward/v1",
      commonAnchorSetDigestHex: hex("7"),
      purgeDurationMinutes: 30,
      embargoDurationMinutes: 30,
      comparisonFamilyId: "fixture",
    };
    for (const [old, next] of [
      ["gaussian-pop-std/v1", "gaussian-pop-std/v2"],
      ["ewma-lambda094/v2", "ewma-lambda094/v3"],
    ]) {
      expect(
        computeTrialIdentityDigestV2({ ...base, baselineId: old! }).equals(
          computeTrialIdentityDigestV2({ ...base, baselineId: next! }),
        ),
      ).toBe(false);
    }
  });
  it("separates harness receipts even when numeric comparisons coincide", () => {
    const input = {
      comparisonFamilyId: no.comparisonFamilyId,
      commonAnchorSetDigestHex: no.commonAnchorSetDigestHex,
      holmComparisons: no.holmComparisons,
      terminalStatus: no.terminalStatus,
    };
    const legacy = createHash("sha256")
      .update(
        [
          "scientific-admission-receipt/v4",
          no.scoringContractVersion,
          no.scoringMetric,
          no.scoringAmendmentDigestHex,
          // Deliberately use the currently unchanged bootstrap version, read from source below.
          readFileSync("lib/trader/research/benchmark/validation-bootstrap-v1.ts", "utf8").match(
  ...
  it("versions only Terminal cache and validates it before interpreting the verdict", () => {
    ...
    expect(source).toContain(
      "reuseScientificEvidenceAsyncV1(PREDICTIVE_TERMINAL_CHECKPOINT_STAGE,",
    );
