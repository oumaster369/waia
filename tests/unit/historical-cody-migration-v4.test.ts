...
describe("DEE-993 protocol-only migration delta", () => {
  it("preserves 0206 completely except exact CDF protocol predicates", () => {
    const before = readFileSync(
      "db/migrations_postgres/0206_historical_brier_admission_v3.sql",
      "utf8",
    );
    const after = readFileSync(
      "db/migrations_postgres/0207_historical_cody_admission_v4.sql",
      "utf8",
    );
    const expected = before
      .slice(before.indexOf("DROP POLICY"))
      .replace(
        "schema_version='scientific-admission-receipt/v3'",
        `schema_version='${SCIENTIFIC_ADMISSION_RECEIPT_V2_VERSION}'`,
      )
    ...
    expect(journal.entries.find((entry: { idx: number }) => entry.idx === 207)).toMatchObject({
      idx: 207,
