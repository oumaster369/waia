...
describe("FHV release identity validator (generic)", () => {
  ...
  it("rejects quoted literal, equals syntax, abbreviated, and wrong variable", () => {
    expect(
  ...
  it("rejects feature-head and checkout literal targets in active sections", () => {
    ...
    expect(
      featureResult.violations.some((v) =>
        ["LITERAL_SHA_IN_ACTIVE_SECTION", "LITERAL_CHECKOUT_TARGET"].includes(v.code),
      ),
    ).toBe(true);
    const checkout =
    ...
    expect(
      validateFhvReleaseIdentityMarkdown(checkout).violations.some(
        (v) => v.code === "LITERAL_CHECKOUT_TARGET",
      ),
    ).toBe(true);
  });

  it("rejects empty target in active sections", () => {
    expect(validateFhvReleaseIdentityMarkdown("--target-sha ").ok).toBe(false);
  });
});
