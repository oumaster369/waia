...
function researchAuthority() {
  return declareResearchNonCapitalInformationAuthorityV2({
    organizationId: ORG,
    reason: "TRADER_PAPER_CANONICAL_RECURRING_CUTOVER_UNIT_TEST",
  });
}
...
function envelope(
  patch: Partial<PaperCanonicalOrdinaryCapitalEnvelopeV2> = {},
): PaperCanonicalOrdinaryCapitalEnvelopeV2 {
...
function submittedOrder(): Extract<SubmitOrderResult, { status: "submitted" }> {
  return {
    ...
    order: {
      ...
      clientOrderId: "client-1024",
      idempotencyKey: "idem-1024",
...
function paperDeps(patch: Partial<PaperCycleDeps> = {}): PaperCycleDeps {
  return {
    execution: {
...
async function runPaper(deps: PaperCycleDeps) {
  return runPaperCycleOnce(deps, {
    context: requireOrgContext(ORG),
    snapshot: snapshot(),
    accountKey: "acct-1024",
...
describe("DEE-1024 paper canonical recurring cutover", () => {
  beforeEach(() => {
