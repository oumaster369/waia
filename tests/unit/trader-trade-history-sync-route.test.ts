...
describe("HTX trade history sync API (DEE-350)", () => {
  ...
  it.each([false, true])("rejects different stored account before data fetch/write (stub=%s)", async (stubValidation) => {
    ...
    const result = await handleTradeHistorySyncPost(
      ...
      syncTradesPostRequest({ symbol: SYNC_SYMBOL }),
      createDeps({
        createTradeHistorySnapshotService: () => ({ recordSnapshot }) as never,
        createConnector: (config) => {
          ...
          vi.spyOn(connector, "getTradeHistory").mockImplementation(read);
          if (stubValidation) {
  ...
  it("preserves generic validation failure when no reason is supplied", async () => {
    ...
    const result = await handleTradeHistorySyncPost(credentialId, syncTradesPostRequest({ symbol: SYNC_SYMBOL }), createDeps({ createConnector: () => connector }));
    expect(result.status).toBe(502);
