...
function trial(modelId: string, terminalTargetDefinitionDigestHex = d("1")) {
  const modelSpec = buildForecastModelSpecV2({ modelId, modelTransformVersion: modelId, inputContractDigestHex: contract.contentDigestHex, terminalTargetDefinitionDigestHex, executionOpportunityTargetDefinitionDigestHex: d("2") });
...
describe("DEE-648 common-anchor challenger arena", () => {
  ...
  it("fails on mixed anchors and target drift", () => {
    ...
    expect(() => runChallengerArenaV2({ submissions:[{...base,trialSpec:trial("a")},{...base,terminalTargetDefinitionDigestHex:d("9"),trialSpec:trial("b", d("9"))}], terminalGrid:{edges:[-3,-2,-1,1,2,3],bucketCount:7}, observedTerminalReturn:0, observedJoint13d:new Array(13).fill(0) })).toThrow("COMMON_TARGET_MISMATCH");
  });
});
