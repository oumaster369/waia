...
import {
  ...
  createHistoricalSimulationCommitRequestV2,
  createHistoricalSimulationModeledAtomicArtifactV2,
  validateHistoricalSimulationCommitRequestV2,
...
} from "@/lib/trader/historical-simulation-v2/production-runtime-state-v2";
...
function ledger(
  ...
  return createHistoricalSimulationReasonLedgerV2({
    ...
    datasetMembership: membership(recordIndex, cycleId),
    previousContentDigestHex: previous?.contentDigestHex ?? null,
    forecast: {
      status: "NON_ACTIONABLE",
      reasonCodes: ["NO_EDGE"],
      authorityContentDigestHex: null,
    },
    decision: {
      status: "CASH",
      reasonCodes: ["NO_EDGE"],
      decisionContentDigestHex: D,
      whyNotCashReceiptDigestHex: D,
      evLower: null,
      evBase: null,
      evUpper: null,
    },
...
function inMemoryRepository(failurePoint: FailurePoint = null) {
  const durable: {
    ledger: HistoricalSimulationReasonLedgerV2[];
    cursor: HistoricalSimulationResumeCursorV2 | null;
  ...
  const repository: HistoricalSimulationAtomicCycleRepositoryV2 = {
    async transaction<T>(
      callback: (tx: HistoricalSimulationAtomicCycleTransactionV2) => Promise<T>,
    ) {
      const staged = {
        ledger: [...durable.ledger],
        cursor: durable.cursor,
        stages: [...durable.stages],
      };
      const tx: HistoricalSimulationAtomicCycleTransactionV2 = {
        async loadLedgerChain() {
        ...
        async persistStageBundle(bundle) {
          ...
          if (failurePoint === "STAGE_AFTER_WRITE")
            throw new Error("INJECTED_CRASH_AFTER_STAGE_WRITE");
        },
        async appendLedger(entry) {
          staged.ledger.push(entry);
          if (failurePoint === "APPEND_AFTER_WRITE")
            throw new Error("INJECTED_CRASH_AFTER_LEDGER_WRITE");
        },
        async saveResumeCursor(cursor) {
          staged.cursor = cursor;
          if (failurePoint === "CURSOR_AFTER_WRITE")
            throw new Error("INJECTED_CRASH_AFTER_CURSOR_WRITE");
        },
      };
...
function snapshots(cycleId: string, sequence: number) {
  ...
  const accountingBody = {
    schemaVersion: "htr-accounting-frontier/v1" as const,
    engineId: "CANONICAL_CROSS_BACKEND_ACCOUNTING_ENGINE_V1" as const,
    basisMethod: "DUAL_GROSS_NET_WEIGHTED_AVERAGE_BASIS_V1" as const,
    organizationId: scope.organizationId,
    accountKey: scope.accountId,
    runId: scope.runId,
    accountingSequence: sequence,
    frontierAsOf: new Date(1_700_000_000_000 + sequence * 60_000).toISOString(),
    monthKey: "2023-11",
    cash: "1000.00000000",
    positions: {},
    ...
    marks: {},
    markedPositionValue: "0.00000000",
    equity: "1000.00000000",
    equityHwm: "1000.00000000",
    accountDrawdownBps: 0,
    consumedFillIds: [],
    id: `frontier-${sequence}`,
    sourceFillId: null,
    sourceEconomicsDigest: D,
    idempotencyKey: `frontier-${sequence}`,
  };
  const accounting = {
    ...accountingBody,
  ...
  return {
    knowledgeSnapshot: createHistoricalSimulationDurableStateSnapshotV2({
      ...
      state: {
        ...
        checkpointContentDigestHex: "b".repeat(64),
...
function stageBundles(cycleId: string, ledgerEntryContentDigestHex: string) {
  ...
  return Object.fromEntries(
    HISTORICAL_SIMULATION_ATOMIC_STAGES_V2.map((stage) => [
      stage,
      createHistoricalSimulationAtomicStageBundleV2({
        ...scope,
        stage,
        cycleId,
        ledgerEntryContentDigestHex,
        artifacts: [
          {
            artifactKind: kinds[stage],
            artifactId: `${cycleId}:${stage}`,
...
describe("Historical Simulation V2 atomic cycle commit and durable resume foundation", () => {
  it("prepares inception ports with null cursor on the exact transaction before producer composition", async () => {
    ...
    const request = createHistoricalSimulationCommitRequestV2({
      ...scope,
      cycleSequence: 0,
      cycleId: first.cycleId,
      replayBarClosedAtUtc: first.replayBarClosedAtUtc,
      datasetMembership: first.datasetMembership,
      datasetMembershipContentDigestHex: first.datasetMembership.contentDigestHex,
      forecastInputAuthorityContentDigestHex: "1".repeat(64),
      policyConfigContentDigestHex: "2".repeat(64),
      codeSha: "3".repeat(40),
      ledgerEntryContentDigestHex: first.contentDigestHex,
      stageBundleDigestHexByStage: Object.fromEntries(
        HISTORICAL_SIMULATION_ATOMIC_STAGES_V2.map((stage) => [
          stage,
          bundles[stage].contentDigestHex,
        ]),
      ) as HistoricalSimulationResumeCursorV2["cycleStageBundleDigestHexByStage"],
      snapshotContentDigestHexByKind: {
        KNOWLEDGE: persisted.knowledgeSnapshot.contentDigestHex,
        ...
        GUARDIAN: persisted.guardianSnapshot.contentDigestHex,
        LEARNING: persisted.learningSnapshot.contentDigestHex,
      },
    });
    ...
    const prepared = await prepareHistoricalSimulationProductionPortsV2({
      ...
      request,
      scope,
      createPorts(receivedTx, previousCursor) {
        return { receivedTx, previousCursor };
      },
    });
  ...
  it("loads and passes the exact validated six-snapshot resume cursor before composing ports", async () => {
    ...
    const request = createHistoricalSimulationCommitRequestV2({
      ...
      cycleSequence: 1,
      cycleId: "cycle-1",
      replayBarClosedAtUtc: "2026-01-01T00:02:00.000Z",
      datasetMembership: nextMembership,
      datasetMembershipContentDigestHex: nextMembership.contentDigestHex,
      forecastInputAuthorityContentDigestHex: "1".repeat(64),
      policyConfigContentDigestHex: "2".repeat(64),
      codeSha: "3".repeat(40),
      ledgerEntryContentDigestHex: "4".repeat(64),
    ...
    const query = async (strings: TemplateStringsArray) => {
      ...
      events.push(
        text.includes("resume_checkpoint_v2")
          ? "checkpoint"
          : text.includes("resume_stage_link_v2")
            ? "stages"
            : text.includes("resume_snapshot_link_v2")
              ? "snapshots"
              : "query",
      );
      ...
      if (text.includes("resume_stage_link_v2 l"))
        return HISTORICAL_SIMULATION_ATOMIC_STAGES_V2.map((stage) => ({
          stage,
          bundle_content_digest_hex: bundles[stage].contentDigestHex,
          cycle_id: first.cycleId,
          ledger_entry_content_digest_hex: first.contentDigestHex,
          artifacts_json: bundles[stage].artifacts,
        }));
      if (text.includes("resume_snapshot_link_v2 l")) {
        const values = {
          KNOWLEDGE: persisted.knowledgeSnapshot,
          MODELED_EXECUTION_REGISTRY: persisted.modeledExecutionRegistrySnapshot,
          MODELED_EXCHANGE: persisted.modeledExchangeSnapshot,
          ACCOUNTING_FRONTIER: persisted.accountingFrontierSnapshot,
          GUARDIAN: persisted.guardianSnapshot,
          LEARNING: persisted.learningSnapshot,
        } as const;
        return Object.entries(values).map(([state_kind, snapshot]) => ({
          state_kind,
          snapshot_content_digest_hex: snapshot.contentDigestHex,
    ...
    const tx = ((strings: TemplateStringsArray) =>
      query(strings)) as unknown as import("postgres").Sql;
    const prepared = await prepareHistoricalSimulationProductionPortsV2({
      tx,
      request,
      scope,
      createPorts(receivedTx, previousCursor) {
        events.push("createPorts");
        return { receivedTx, previousCursor };
      },
    });
    ...
    expect([
      prepared.previousCursor?.knowledgeSnapshot,
      prepared.previousCursor?.modeledExecutionRegistrySnapshot,
      prepared.previousCursor?.modeledExchangeSnapshot,
      prepared.previousCursor?.accountingFrontierSnapshot,
      prepared.previousCursor?.guardianSnapshot,
      prepared.previousCursor?.learningSnapshot,
    ]).toEqual([
      persisted.knowledgeSnapshot,
      persisted.modeledExecutionRegistrySnapshot,
      persisted.modeledExchangeSnapshot,
      persisted.accountingFrontierSnapshot,
      persisted.guardianSnapshot,
      persisted.learningSnapshot,
    ]);
  });
  it("restores all mutable production runtime components from the exact durable cursor", async () => {
    const first = ledger(0, null);
    const persisted = await commit(inMemoryRepository().repository, first);
    const runtime = restoreHistoricalSimulationProductionRuntimeStateV2({
      scope,
      cursor: persisted,
    });
    ...
    expect(runtime.accounting.semanticContentDigest).toBe(
      (persisted.accountingFrontierSnapshot.state as { semanticContentDigest: string })
    ...
    const roundTrip = snapshotHistoricalSimulationProductionRuntimeStateV2({
      scope,
      cycleId: persisted.committedCycleId,
      runtime,
    });
    expect(roundTrip).toEqual({
      ...
      guardianSnapshot: persisted.guardianSnapshot,
      learningSnapshot: persisted.learningSnapshot,
    });
  });
  it("loads deterministic inception accounting only through its exact 0187 authority identity", async () => {
    const inception = createInitialAccountingState({
      organizationId: scope.organizationId,
      accountKey: scope.accountId,
      runId: scope.runId,
      startingCash: "1000.00000000",
      frontierAsOf: "2023-11-14T22:14:20.000Z",
    });
    const accounting = {
      ...inception,
      ...
      sourceFillId: null,
      sourceEconomicsDigest: D,
      semanticContentDigest: computeAccountingSemanticDigest(inception),
      idempotencyKey: "frontier-inception",
    } as Record<string, unknown>;
    const bundle = {
      schemaVersion: "waia.trader.dee659_authority_preregistration.v2",
      initialAccountingIdentity: {
        id: accounting.id,
    ...
    const row = {
      authority_bundle_json: bundle,
      authority_bundle_digest_hex: bundleDigest,
      id: accounting.id,
      organization_id: accounting.organizationId,
      account_key: accounting.accountKey,
      run_id: accounting.runId,
      accounting_sequence: 1,
      frontier_as_of: accounting.frontierAsOf,
      cash: accounting.cash,
      position_quantity_json: {},
      gross_position_basis_json: {},
      net_position_basis_json: {},
      gross_realized_pnl: accounting.grossRealizedPnl,
      net_realized_pnl: accounting.netRealizedPnl,
      marks_json: {},
      equity: accounting.equity,
      equity_hwm: accounting.equityHwm,
      account_drawdown_bps: 0,
      source_fill_id: null,
      source_economics_digest: accounting.sourceEconomicsDigest,
      semantic_content_digest: accounting.semanticContentDigest,
      idempotency_key: accounting.idempotencyKey,
      schema_version: accounting.schemaVersion,
    };
    const tx = (async () => [row]) as unknown as import("postgres").Sql;
    const loaded = await loadHistoricalSimulationInceptionAccountingV2({
      tx,
      scope,
    ...
    ).rejects.toThrow("INCEPTION_ACCOUNTING");
  });
  it("content-addresses every immutable commit input and full modeled payload", () => {
    ...
    const request = createHistoricalSimulationCommitRequestV2({
      ...
      snapshotContentDigestHexByKind: {
        KNOWLEDGE: persistedInput.knowledgeSnapshot.contentDigestHex,
        MODELED_EXECUTION_REGISTRY:
          persistedInput.modeledExecutionRegistrySnapshot.contentDigestHex,
    ...
    for (const mutation of [
      { cycleId: "changed" },
      { datasetMembershipContentDigestHex: "4".repeat(64) },
      { forecastInputAuthorityContentDigestHex: "5".repeat(64) },
      { policyConfigContentDigestHex: "6".repeat(64) },
      { codeSha: "7".repeat(40) },
      { ledgerEntryContentDigestHex: "8".repeat(64) },
    ])
      expect(() =>
        validateHistoricalSimulationCommitRequestV2({ ...request, ...mutation }),
      ).toThrow("COMMIT_REQUEST");
    const sourcePayload = { posture: "NONE" };
    const modeled = createHistoricalSimulationModeledAtomicArtifactV2({
      artifactKind: "GUARDIAN_ASSESSMENT",
      artifactId: "guardian-0",
      ...scope,
      cycleId: first.cycleId,
      pitAnchor: first.replayBarClosedAtUtc,
      sourceContentDigestHex: computeSemanticSha256Hex(sourcePayload),
      sourcePayload,
    });
    ...
    expect(
      validateHistoricalSimulationModeledAtomicArtifactV2(scope, first.cycleId, {
        artifactKind: modeled.artifactKind,
        artifactId: modeled.artifactId,
    ...
    const learning = createHistoricalSimulationModeledAtomicArtifactV2({
      artifactKind: "LEARNING_UPDATE",
      artifactId: "learning-0",
      ...scope,
      cycleId: first.cycleId,
      pitAnchor: first.replayBarClosedAtUtc,
      sourceContentDigestHex: computeSemanticSha256Hex({
        status: "NO_UPDATE",
        previousState: null,
      ...
      lineagePayload: {
        previousSnapshotContentDigestHex: null,
        nextSnapshotContentDigestHex: learningSnapshot.contentDigestHex,
        previousState: null,
        nextState: learningSnapshot.state,
      },
    });
    expect(() =>
      assertHistoricalSimulationLearningSnapshotTransitionV2({
        ...
        nextSnapshot: learningSnapshot,
        artifacts: [
          {
            ...learning,
            lineagePayload: {
              ...learning.lineagePayload,
              nextState: {
                ...learningSnapshot.state,
                pendingForecastAuthorityContentDigestHexes: ["9".repeat(64)],
              },
            },
          },
        ],
  ...
  it("atomically advances the exact ledger, dataset, knowledge, registry and accounting frontier", async () => {
    ...
    expect(loadValidatedHistoricalSimulationLedgerHeadV2(memory.durable.ledger, scope)).toEqual(
      first,
    );
  ...
  it("accepts a qualified nonzero first record and requires exact cursor continuity thereafter", async () => {
    const memory = inMemoryRepository();
    const first = ledger(0, null, 239);
    const cursor0 = await commit(memory.repository, first);
    expect(cursor0.nextCycleSequence).toBe(1);
    expect(cursor0.nextRecordIndex).toBe(240);

    const second = ledger(1, first, 240);
    const cursor1 = await commit(memory.repository, second);
    expect(cursor1.nextCycleSequence).toBe(2);
    expect(cursor1.nextRecordIndex).toBe(241);

    const gapMemory = inMemoryRepository();
    const gapFirst = ledger(0, null, 239);
    await commit(gapMemory.repository, gapFirst);
    await expect(commit(gapMemory.repository, ledger(1, gapFirst, 241))).rejects.toThrow(
      "NEXT_SEQUENCE_OR_BINDING:RECORD_INDEX",
    );
  });
  ...
  it.each(["STAGE_AFTER_WRITE", "APPEND_AFTER_WRITE", "CURSOR_AFTER_WRITE"] as const)(
    ...
    async (failurePoint) => {
      ...
      await expect(commit(retry.repository, recomputedAfterRollback)).resolves.toMatchObject({
        nextCycleSequence: 1,
      });
  ...
  it("returns the durable cursor on an exact retry and rejects a changed sealed dataset authority", async () => {
    ...
    expect(() =>
      assertHistoricalSimulationResumeAtMembershipV2({
        cursor,
        scope,
        membership: { ...membership(1), partitionDigestHex: "f".repeat(64) },
      }),
  ...
  it("restores an outstanding modeled order and produces the identical next-bar fill", async () => {
    const openOrder = {
      orderId: "order-1",
      acceptedAtTs: 1_700_000_000_000,
      firstEligibleTs: 1_700_000_060_000,
      windowEndBarIndex: 3,
      sameSymbolEligibleBarsSeen: 0,
      remainingQty: "1.00000000",
      filledQty: "0.00000000",
      fillSequence: 0,
    };
    const durableOrderBody = {
      id: "order-1",
      organizationId: scope.organizationId,
      credentialId: null,
      venue: "HTX",
    ...
    const durableOrder = {
      ...durableOrderBody,
      contentDigestHex: computeSemanticSha256Hex(durableOrderBody),
    };
    ...
    const receiptBody = {
      ...
      orderId: "order-1",
      orderContentDigestHex: computeSemanticSha256Hex({
    ...
    ).toThrow("MODELED_EXECUTION_REGISTRY_STATE");
    ...
    const cursor = await commitHistoricalSimulationCycleAtomicallyV2({
      ...
      scope,
      ledgerEntry: first,
      stageBundles: stageBundles(first.cycleId, first.contentDigestHex),
      knowledgeCheckpointSequence: 0,
      knowledgeCheckpointContentDigestHex: "b".repeat(64),
      ...snapshots(first.cycleId, 0),
      modeledExchangeSnapshot,
      modeledExecutionRegistrySnapshot,
    });
    ...
    const restoredOrders = restoreHistoricalModeledExchangeOrdersV2(
      cursor.modeledExchangeSnapshot as never,
      { ...scope, cycleId: first.cycleId },
    );
    ...
    const productionRuntime = restoreHistoricalSimulationProductionRuntimeStateV2({
      scope,
      cursor,
    });
    ...
    const nonEmptyRoundTrip = snapshotHistoricalSimulationProductionRuntimeStateV2({
      scope,
      cycleId: cursor.committedCycleId,
      runtime: productionRuntime,
    });
    expect(nonEmptyRoundTrip.modeledExecutionRegistrySnapshot).toEqual(
      cursor.modeledExecutionRegistrySnapshot,
    );
    ...
    await expect(
      commitHistoricalSimulationCycleAtomicallyV2({
        ...
        knowledgeCheckpointContentDigestHex: "b".repeat(64),
        ...snapshots(first.cycleId, 0),
        modeledExchangeSnapshot: inconsistentExchange,
        modeledExecutionRegistrySnapshot,
      }),
  ...
  it("refuses a typed-stage substitution and an account splice before persistence", async () => {
    ...
    expect(() =>
      createHistoricalSimulationAtomicStageBundleV2({
        ...
        ledgerEntryContentDigestHex: first.contentDigestHex,
        artifacts: [
          { artifactKind: "GUARDIAN_ASSESSMENT", artifactId: "forged", contentDigestHex: D },
        ],
      }),
    ...
    await expect(
      commitHistoricalSimulationCycleAtomicallyV2({
        repository: memory.repository,
        scope: { ...scope, accountId: "another-account" },
        ledgerEntry: first,
        stageBundles: stageBundles(first.cycleId, first.contentDigestHex),
        knowledgeCheckpointSequence: 0,
        knowledgeCheckpointContentDigestHex: "b".repeat(64),
  ...
  it("deep-freezes durable state and refuses a forged accounting semantic digest", () => {
    ...
    const forged = createHistoricalSimulationDurableStateSnapshotV2({
      ...scope,
      cycleId: "cycle-0",
      stateKind: "ACCOUNTING_FRONTIER",
      state: {
        ...(accounting.state as Record<string, unknown>),
        semanticContentDigest: D,
      },
    } as never);
    expect(() =>
      validateHistoricalSimulationDurableStateSnapshotV2(forged, "ACCOUNTING_FRONTIER"),
  ...
  it("detects an exact committed retry before constructing ports or invoking producers", async () => {
    ...
    const request = createHistoricalSimulationCommitRequestV2({
      ...
      ) as typeof persisted.cycleStageBundleDigestHexByStage,
      snapshotContentDigestHexByKind: {
        KNOWLEDGE: persisted.knowledgeSnapshot.contentDigestHex,
        MODELED_EXECUTION_REGISTRY: persisted.modeledExecutionRegistrySnapshot.contentDigestHex,
        MODELED_EXCHANGE: persisted.modeledExchangeSnapshot.contentDigestHex,
    ...
    const query = async (strings: TemplateStringsArray) => {
      const text = strings.join("?");
      if (text.includes("FROM trader_historical_dataset_authority_v2 d"))
        return [
          {
      ...
      if (text.includes("SELECT forecast_json FROM trader_historical_simulation_reason_ledger_v2"))
        return [{ forecast_json: { status: "AUTHORIZED" } }];
      if (text.includes("FROM trader_dee659_authority_preregistration_v2"))
        return [
          {
      ...
      if (text.includes("commit_request_digest_hex,commit_request_json"))
        return [
          {
            checkpoint_json: persisted,
            commit_request_digest_hex: request.contentDigestHex,
            commit_request_json: request,
            committed_cycle_id: first.cycleId,
            committed_cycle_sequence: 0,
            ledger_head_content_digest_hex: first.contentDigestHex,
          },
        ];
      if (text.includes("SELECT checkpoint_json, committed_cycle_sequence")) {
        return [{ checkpoint_json: persisted, committed_cycle_sequence: 0 }];
    ...
    type FakeSqlExtensions = {
      begin: (
        _level: string,
        callback: (sql: import("postgres").Sql) => Promise<unknown>,
      ) => Promise<unknown>;
    ...
    (fake as unknown as FakeSqlExtensions).reserve = async () => {
      reservations += 1;
      (fake as unknown as FakeSqlExtensions).release = () => {
        releases += 1;
      };
      return fake;
    };
    let portsCreated = 0;
    let produced = 0;
    const result = await commitHistoricalSimulationCyclePostgresV2({
      sql: fake,
      scope,
      request,
      createPorts() {
    ...
    expect({ portsCreated, produced, reservations, releases }).toEqual({
      portsCreated: 0,
      produced: 0,
      reservations: 1,
      releases: 1,
    ...
    await expect(
      commitHistoricalSimulationCyclePostgresV2({
        sql: fake,
        scope,
        request: createHistoricalSimulationCommitRequestV2({
          ...requestSeed,
          policyConfigContentDigestHex: "8".repeat(64),
        }),
        createPorts: () => ({}),
        async produce() {
          throw new Error("must not run");
    ...
    expect({ reservations, releases }).toEqual({ reservations: 2, releases: 2 });

    delete (fake as unknown as Partial<FakeSqlExtensions>).reserve;
    const heldResult = await commitHistoricalSimulationCyclePostgresV2({
      sql: fake,
      scope,
      request,
      createPorts() {
    ...
    expect(heldResult).toEqual(persisted);
    expect({ reservations, releases }).toEqual({ reservations: 2, releases: 2 });
  });
});
