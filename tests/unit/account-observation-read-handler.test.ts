...
describe("shared account observation HTTP boundary (injected auth, no production calls)", () => {
  it("requires session and never opens the store on denial", async () => {
    const d = deps();
    vi.mocked(d.getUserId).mockResolvedValue(null);
    expect((await handleAccountObservationGet(request(), "tenant", d)).status).toBe(401);
    expect(d.readLatest).not.toHaveBeenCalled();
  });
  it.each(["hasTraderAccess", "hasOperatorAccess"] as const)(
    "fails closed on admin %s denial",
    async (key) => {
      const d = deps();
      vi.mocked(d[key]).mockResolvedValue(false);
      expect((await handleAccountObservationGet(request(), "admin", d)).status).toBe(403);
      expect(d.readLatest).not.toHaveBeenCalled();
    },
  );
  it("admin reads without target-org membership when operator permission holds", async () => {
    const d = deps();
  ...
  it("tenant does not need operator, missing remains HTTP204 not an empty balance", async () => {
    const d = deps();
    vi.mocked(d.hasOperatorAccess).mockResolvedValue(false);
  ...
  it("stored scope mismatch rejects caller-selected account", async () => {
    const d = deps();
    vi.mocked(d.resolveActiveBinding).mockResolvedValue({ ...binding, exchangeAccountId: "other" });
    expect((await handleAccountObservationGet(request(), "tenant", d)).status).toBe(403);
    expect(d.readLatest).not.toHaveBeenCalled();
  });
  ...
  it("does not serialize private/raw dependency failure", async () => {
    const d = deps();
    vi.mocked(d.readLatest).mockRejectedValue(new Error("synthetic-private-driver-detail"));
    const response = await handleAccountObservationGet(request(), "tenant", d);
    ...
    expect(
      (
        await handleAccountObservationGet(
          new Request(request().url + "&credentialId=other"),
          "tenant",
          d,
        )
      ).status,
  ...
  it("an already aborted request does not read account data", async () => {
    ...
    const d = deps();
    await handleAccountObservationGet(
      new Request(request().url, { signal: controller.signal }),
      "tenant",
      d,
    );
    expect(d.readLatest).not.toHaveBeenCalled();
  ...
  it("tenant binding discovery derives organization from verified identity, not query input", async () => {
    const d = deps();
    const expected = { ...binding, organizationId: personalOrganizationIdFromUserId("user") };
    ...
    expect(d.hasTraderAccess).toHaveBeenCalledWith(
      "user",
    ...
    expect(
      (
        await handleAccountObservationGet(
          new Request(url + "&organizationId=" + binding.organizationId),
          "tenant",
          d,
          "binding",
        )
  ...
  it("admin binding discovery checks the exact selected organization", async () => {
    ...
    expect(response.status).toBe(200);
    expect(d.hasOperatorAccess).toHaveBeenCalledWith(
      "user",
  ...
  it("rotation during storage read discards the entire old response", async () => {
    const d = deps();
    vi.mocked(d.readLatest).mockImplementation(async () => {
      vi.mocked(d.resolveActiveBinding).mockResolvedValue({ ...binding, credentialRevision: "2" });
      return null;
    });
    expect((await handleAccountObservationGet(request(), "tenant", d)).status).toBe(403);
  });
  it("bounds a stalled dependency without exposing data", async () => {
    ...
    try {
      const d = deps();
      vi.mocked(d.readLatest).mockImplementation(() => new Promise(() => {}));
