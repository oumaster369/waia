...
const USER_A = "00000000-0000-4000-8000-00000000a350";
const USER_B = "00000000-0000-4000-8000-00000000b350";
...
describe("HTX position sync tenant isolation (DEE-350)", () => {
  ...
  beforeAll(async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "waia-htx-position-iso-"));
    process.env.DATABASE_URL = `file:${path.join(tmpDir, "htx-position-iso.sqlite")}`;
    migrateDatabaseFromEnv();
    const db = getDb();

    insertEmailPasswordUser(db, {
      id: USER_A,
      email: "htx-position-iso-a@waia.invalid",
      password: "password123",
      identityLabel: "HTX Org A",
    });
    insertEmailPasswordUser(db, {
      id: USER_B,
      email: "htx-position-iso-b@waia.invalid",
      password: "password123",
    ...
    const syncResult = await handlePositionSyncPost(credentialId, createDepsForUser(USER_A));
    expect(syncResult.status).toBe(200);
  });

  function createDepsForUser(userId: string): PositionSyncHandlerDeps {
  ...
  it("user B GET list does not include user A snapshots", async () => {
    const result = await handlePositionSnapshotsGet(
  ...
  it("user A GET list includes their snapshots only", async () => {
    const result = await handlePositionSnapshotsGet(
      new Request(`http://localhost/api/trader/position-snapshots?credentialId=${credentialId}`),
      createDepsForUser(USER_A),
    );
    expect(result.status).toBe(200);

    const body = result.body as {
      snapshots: Array<{ credentialId: string; positions: unknown[] }>;
    };
  ...
  it("user B cannot sync user A credential", async () => {
    const result = await handlePositionSyncPost(credentialId, createDepsForUser(USER_B));
