    ...
    if (command === "refresh_from_categories") {
      return withTreasuryAdmin({
        ...
        fn: async ({ userId, services }) => {
          ...
          const derived = await services.ledgerCatalog.deriveAnnualBudgetMicros(
            context,
            currency,
            periodYear,
          );
  ...
  try {
    ...
    return withTreasuryAdmin({
      ...
      fn: async ({ services }) => {
        ...
        return adminSuccess({
          month: serializeCategoryBudgetMonth(
            await services.ledgerCatalog.getBudgetMonthSummary(context, month),
          ),
        });
      },
    });
        ...
        switch (kind) {
          ...
          case "categories":
            if (body.code !== undefined) {
              throw new TreasuryValidationError(
                "CATEGORY_CODE_SERVER_OWNED",
                "category code is generated by the server",
              );
            }
            return adminSuccess({
            ...
            return adminSuccess({
              category: serializeCategory(
                await services.ledgerCatalog.updateCategory(context, admin, id, {
                  ...
                  groupName:
                    body.group_name === undefined && body.groupName === undefined
                      ? undefined
                      : requireString(body.group_name ?? body.groupName, "group_name"),
                  description:
                  ...
                  currency:
                    body.currency === undefined
                      ...
                      : requireString(body.currency, "currency"),
                  effectiveMonth:
                    body.effective_month === undefined && body.effectiveMonth === undefined
                      ? undefined
                      : requireString(
                          body.effective_month ?? body.effectiveMonth,
                          "effective_month",
