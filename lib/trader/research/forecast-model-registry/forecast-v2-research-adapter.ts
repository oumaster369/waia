...
import {
  ...
  type ForecastInputIdentitiesV2,
...
export function adaptResearchForecastV2(input: {
  ...
  inputIdentities: ForecastInputIdentitiesV2;
  terminalBucketProbabilities: readonly number[];
  joint13dSamples: readonly (readonly number[])[];
  ...
  const body = {
    ...
    anchorClosedBarAt: input.inputIdentities.anchorClosedBarAt,
    terminalBucketProbabilities: [...input.terminalBucketProbabilities],
    joint13dSamples: input.joint13dSamples.map((row) => [...row]),
    researchOnly: true as const,
  };
  return { ...body, contentDigestHex: computeSemanticSha256Hex({ ...body, modelSpecDigestHex: spec.contentDigestHex, artifactDigestHex: artifact.contentDigestHex, qualifiedInputBindingDigestHex: input.inputIdentities.qualifiedInputBindingDigestHex }) };
}
