/** Cody CALERF erf kernel (`cdf-erf-cody715/v2`, JINT=0). */
export const CDF_ERF_CODY715_VERSION = "cdf-erf-cody715/v2" as const;
...
export function erfCody715V2(x: number): number {
  ...
  let result = 0;
  // CALERF JINT=0 saturates here; the scaled-erfc branch is JINT=2 only.
  if (absY >= XBIG) return fixupErfFromErfc(x, 0);
...
export function normalCdfCody715V2(z: number): number {
  return HALF * (ONE + erfCody715V2(z / SQRT2));
}
