...
export function evaluateMeanReversionV0(
  ...
  if (compareDecimal(zscore, ZSCORE_BUY_THRESHOLD) <= 0) {
    return {
      ...
      expectedEdge: multiplyDecimal(absDecimal(zscore), features.features.priceDispersion20),
  ...
  if (compareDecimal(zscore, ZSCORE_SELL_THRESHOLD) >= 0) {
    return {
      ...
      side: "sell",
      confidence: "0.55",
      expectedEdge: multiplyDecimal(features.features.priceDispersion20, "0.25"),
      horizon: DEFAULT_HORIZON,
      maxRisk: DEFAULT_MAX_RISK,
