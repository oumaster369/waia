...
export async function readOverviewSnapshot(db: AdminPostgresDb, input: OverviewReadInput) {
  return withAdminReadSnapshot(db, (tx) => readOverviewWithinSnapshot(tx, input));
}
/** Reuse inside an existing financial snapshot; callers must not open a second transaction. */
export async function readOverviewWithinSnapshot(tx: AdminReadTx, input: OverviewReadInput) {
