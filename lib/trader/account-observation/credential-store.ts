 ...
 */
export function createObservationCredentialStore(
  input: Readonly<{
  ...
  return Object.freeze({
    async openCredential(
      ...
      signal: AbortSignal,
    ): Promise<HtxObservationCredentialHandle> {
      if (disposed || active || handles.size >= 20 || signal.aborted) fail("READ_FAILED");
      let binding: ObservationBinding;
      try {
        binding = Object.freeze(observationBindingSchema.parse(requested));
      } catch {
      ...
      active = controller;
      let abandoned = false;
      let created: HtxObservationCredentialHandle | undefined;
      ...
      const current = () => {
        if (disposed || abandoned || controller.signal.aborted) fail("READ_FAILED");
      };
      const fence = async () => {
        current();
            ...
            Object.defineProperties(
              { binding, dispose },
              {
                apiKey: {
                  enumerable: false,
                  get() {
                    if (released || apiKey === undefined) fail("READ_FAILED");
                    return apiKey;
                  },
                },
                apiSecret: {
                  enumerable: false,
                  get() {
                    if (released || apiSecret === undefined) fail("READ_FAILED");
                    return apiSecret;
                  },
                },
              },
