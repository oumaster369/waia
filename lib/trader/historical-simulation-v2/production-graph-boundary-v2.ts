...
export type HistoricalForecastPitAuthorityReceiptV2 = Readonly<{
  ...
  datasetMembershipContentDigestHex: string; datasetAuthorityDigestHex: string; buildSha: string;
...
const RECEIPT_KEYS = new Set(["schemaVersion", "organizationId", "accountId", "runId", "cycleId", "pitAnchor",
  "datasetMembershipContentDigestHex", "datasetAuthorityDigestHex", "buildSha", "forecastId",
...
export function assertHistoricalForecastPitAuthorityReceiptV2(value: unknown, expected: Readonly<{
  ...
  const digests = [receipt.datasetMembershipContentDigestHex, receipt.datasetAuthorityDigestHex,
  ...
  if (keys.length !== RECEIPT_KEYS.size || [...RECEIPT_KEYS].some((key) => !Object.hasOwn(receipt, key)) ||
      ...
      receipt.datasetAuthorityDigestHex !== expected.datasetAuthorityDigestHex || receipt.buildSha !== expected.buildSha ||
