...
- **Accounting / Cost-Basis doctrine.** *Reserved:* lot matching, cost-basis, full realized/unrealized P&L method. *Partially instantiated:* the [Closed Trade Reality Doctrine (LD-10)](AI-TRADER-CLOSED-TRADE-REALITY-DOCTRINE.md) defines the **realized fee base** (Realized Strategy Profit) and Strategy HWM semantics; lot matching and full cost-basis method remain reserved. *MVP justification:* Billing & HWM owns the only money-facing computation today, gated manually. *Compensating control:* Reality publishes realized cashflow as fact; LD-10 defines what is billable; computation stays in Billing. *Seam:* realized-cashflow records (Section 4) + LD-10 fee-base definition.
