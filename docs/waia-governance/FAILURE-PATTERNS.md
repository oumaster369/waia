...
| **FP-010** *(historical — dual-branch era; superseded by single-trunk `main`, DEE-511)* | **Symptom (historical):** After a release, `main` showed as ahead of / diverged from `dev` and release PRs hit history conflicts. **Root cause (historical):** release-promotion and/or `main → dev` back-sync PRs were **squash-merged** (or skipped); squash cannot preserve the second parent, so `main` never became an ancestor of `dev`. **Historical recovery:** open a `dee-<NN>-release-back-sync-*` PR (`git merge --no-ff origin/main`) and merge with **"Create a merge commit"**; verify `git merge-base --is-ancestor origin/main origin/dev`. **Supersession:** single canonical trunk is **`main`**; feature PRs squash-merge to `main`; official release is an explicit Human tag of a `main` SHA; `dev` is frozen pending Human retirement; promotion/back-sync are **not** active workflow ([`BRANCHING-STRATEGY.md`](BRANCHING-STRATEGY.md), [`POST-MERGE-PROTOCOL.md`](POST-MERGE-PROTOCOL.md)). Do not revive dual-branch ancestry repair as routine. |
| **FP-011** | **Symptom:** PR governance CI fails on missing `**Linear:**` / `**Tier:**` after agent `/prepare-pr` handoff (e.g. PR #217 used plain `Linear: DEE-260`). **Root cause:** agents synthesized paste-ready bodies with plain key-value syntax instead of copying [`.github/pull_request_template.md`](../../.github/pull_request_template.md); validation ran only reactively in CI. **Recovery:** Edit PR body to template markdown syntax. **Prevent:** [`../../.cursor/commands/prepare-pr.md`](../../.cursor/commands/prepare-pr.md) template-copy contract + mandatory [`preflight-pr-governance.sh`](../../scripts/linear/preflight-pr-governance.sh) before handoff. |
