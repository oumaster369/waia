...
- **DEE-72.1 / DEE-72.2 / DEE-72.3 twin/diary/verifications/memory persistence**: [`postgres-twin-persistence.test.ts`](../tests/integration/postgres-twin-persistence.test.ts) exercises **`PostgresTwinPersistence`** (dialogue, diary, scenario, prediction verifications append/list, **read-only memory search**, chronological reads, readiness load, rollback). Same **opt-in** gate (`WAIA_PG_INTEGRATION=1` + `DATABASE_URL_POSTGRES`). **Does not** test production runtime routing.
