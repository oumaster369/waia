...
Final visual review also found untranslated persisted campaign states and known unavailable reasons. Add Russian descriptions while retaining machine reason attributes/tooltips and preserving unknown-code fallback. No state, arithmetic, permission or workflow change.

Production collector preparation found a remaining inconsistent predicate: monetary history and current-period endpoints required the entire account state to be `ok`, rejecting a fully proven balance valuation whenever order observations were partial. A real-Postgres regression fails before the fix. Use the existing complete native monetary valuation proof plus freshness/inclusion guards, preserve observation warnings, and record the monetary projection state separately. Missing asset quotes and failed balance observations still prevent collection; deposits do not become operational PnL. No valuation formula or billing policy changes.

Production quote metadata also demonstrated stale Coinbase USDT/USD alongside a fresh Kraken fallback. Previously source priority selected stale Coinbase for both balance valuation and PnL, despite the fresh collected fallback. Share one FX selector: valid fresh evidence first, preserve Coinbase priority among equally fresh candidates, retain stale evidence and its age only when no fresh source exists. Regression fails before correction; real-Postgres balance/PnL assertions prove the same Kraken conversion and source. No synthetic 1:1 rate or new provider.
