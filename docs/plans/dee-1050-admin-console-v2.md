...
2. **Замер накладных расходов** — `tests/integration/admin-console-change-log-overhead-postgres.test.ts`.
   ...
   - Локальный прогон и первый CI job `admin-console-overhead-postgres` (изолированный `postgres:16`, триггеры сначала). CI, 2026-09-24: p95 порог 6.053 мс / факт 5.546 мс / ok; p99 факт 7.118 мс; WAL порог 9896026 байт / факт 7788312 байт / ok; CI p95 порог 10.088 мс / факт 5.546 мс / ok; CI WAL порог 15224656 байт / факт 7788312 байт / ok. База без триггеров: p95 5.044 мс, p99 7.355 мс, WAL 7612328 байт. Каждый прогон печатает свою таблицу заново.
