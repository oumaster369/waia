...
## Reality consumer evidence refresh

Reviewed the sole changed inventoried consumer, `lib/trader/admin-console/collectors/run-due.ts`: the added task calls the injected `collectValuations` store to persist console valuations from already observed data. It adds no exchange call, Reality ingress or trading/capital authority. Refresh only `consumerDiscovery.sortedContentDigestHex`; the 134 consumer paths, 155 source files, 25 connector references, source digests and all admission rules stay unchanged. The pinned Reality graph validator and its counterexample tests must pass.
