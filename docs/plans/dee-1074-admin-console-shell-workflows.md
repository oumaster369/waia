...
- Full CI exposed obsolete browser selectors and the retired runtime-authority route fixture. The fixture now exercises the canonical System authority table, retains HALT reason visibility and absence of commands, and verifies real sign-out authorization revocation. Fifteen host/runtime browser scenarios pass, as do eight schema/metadata Postgres and unit checks. Runtime reason codes are selected explicitly from the saved assessment; no raw canonical payload is exposed.
