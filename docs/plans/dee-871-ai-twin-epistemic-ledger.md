...
Every read/write/retry uses the existing short per-subject transaction lock and authoritative current source eligibility. Reuse approved model retention with a creation-anchored annual necessity review; no caller-controlled anchor or retention extension. Refuse current use when the relation interval has ended, sources are unavailable or the model review is due. A shape-valid candidate is not a verified relationship or resolved question. Immutable content plus scope/purpose/request-bound fingerprint receipts prevents conflicting retries; semantically identical JSON key ordering must not cause a conflict. Clock rollback must not admit a new record. No external work inside a transaction.
