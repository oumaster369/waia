...
The renewed review found one remaining P2: the database and exported quarantine intent could label an existing exact correction target as missing. The Human authorized the single correction on 2026-08-22 from clean head `b73677c61cf48af2015ef61a4f1494b71001e70d` on the unchanged base. The B→C remediation now defines the same current-target predicate in ingest, repository, and `0160`: exact organization/account, source kind, native identity kind/id, referenced native revision, subject, empty markers, an admitted `OBSERVED`/`SUPERSEDED` event, and no later `SUPERSEDED` event. The existing per-scope transaction advisory lock also guards direct truth insertion so target creation and absence decisions serialize.

Final focused evidence is green: all five Reality V2 focused unit files pass 25/25, typecheck and scoped lint pass, a fresh disposable PostgreSQL database applies every migration through amended unpublished `0160`, and the Reality PostgreSQL adversarial suite passes 8/8. Negative proofs cover exact-identity repository rejection, direct-SQL rejection, same-revision/different-identity allowance, and a concurrent target-write/quarantine race. The serialized remediation implementation head is `d62b34f93ca8809c7d0423b1d5390c8686ed2d83`; this provenance seal refreezes the exact diff for one fresh independent exact-head review before the still-unused full-suite allowance.

That review found one final in-scope P2: the generic unverifiable quarantine route could reuse the correction-only `CORRECTION_TARGET_NOT_FOUND` reason without a complete correction identity, including in a mixed reason set. Human/controller authorization on 2026-08-22 resumes the same train from clean head `b0e42eb4fe3839eb5d1d4a708b479fa737f828a1` on the unchanged base. The repository and amended unpublished `0160` now reserve that reason to a singleton on a persisted `VERIFIED`/`ATTRIBUTED` source with complete native identity, native revision, supersedes revision, and the existing exact current-target absence predicate. The generic `UNVERIFIABLE` branch rejects every reason set containing the reserved code.

Focused evidence is green: scoped lint and typecheck pass; migration/contracts/projection unit coverage passes 17/17; a fresh disposable PostgreSQL database applies every migration through amended `0160`; and the Reality PostgreSQL adversarial suite passes 9/9. Repository and direct-SQL negative proofs reject reserved-reason attempts from unverifiable sources, verified non-corrections, and mixed reason sets. The remediation implementation head is `2eb37ecba162251627046e19ebb26cb258334304`; this provenance seal refreezes the exact diff for one fresh independent exact-head review before the single changed-head full-suite allowance.
