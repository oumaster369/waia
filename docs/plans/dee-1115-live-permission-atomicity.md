...
## Accepted-base integration — PR674 main

Integrate accepted main `ed2a25f72008a97211c9454fd29d4f62a65508b2` into the existing DEE-1115 branch. Preserve the production service and 48-case native suite byte-for-byte from `507369ea9f4061c69fd3031c0674ebd9a9c39499`. The only merge conflicts are the additive required-suite lists and their proof-guard test count: retain all twelve incoming suites and add live-permission atomicity as the thirteenth. Incoming billing fixes, non-capital owner, migration0218 and self-hosted Manrope are not reimplemented or altered.

On integration code head `d4b6e3c1a997e5f1453809d29646f888141098ea`, the auth prelude and all219 migrations passed on newly created `waia_dee1115_ed2a25f7` (loopback54329, PostgreSQL16.14), starting with zero public tables. All263 native tests across thirteen mandatory suites passed with zero skips; the executed-proof guard passed. Focused acceptance passed50 tests across eight unit files, including36 existing live/authorization cases and14 proof-guard cases. Scoped ESLint and the diff check against accepted main passed. Final inspection found zero other database sessions and zero fixture fault triggers; the database grant was released.

Evidence is retained under the completion audit's `evidence/dee-1115/accepted-base-ed2a25f7/`: `acceptance.json`, native/unit JSON results and logs, and `source-preservation.json`. The latter verifies unchanged service/test blobs, all25 nonconflicting incoming files and the exact six-file publication delta. The nonauthor reviewed the integration source without a blocker; final evidence-delta acceptance remains separate. Full readiness and publication remain controller-owned, with fresh CI required on the eventual PR head. No real account, production permission or C3 calculation was used.
