...
state:
  status: in-progress
  humanApproval: MACRO_E_PACKET_HUMAN_APPROVED_PENDING_PROFILE_DIGEST_REBIND   # 2026-07-15: six substantive Macro-E decisions consumed at 04386b5; digest-integrity reconciliation removes governance-only fields from runtime profile/matrix JSON; narrow Human digest rebind required before Build. HISTORICAL/CONSUMED: AUTHORIZE_OPUS_MACRO_D_PHASE_B; AUTHORIZE-HTR-WP13-WP16-INTELLIGENCE-TRANCHE-PLANNING; AUTHORIZE-HTR-MACRO-E-FINAL-PREAPPROVAL-RECONCILIATION; AUTHORIZE-HTR-MACRO-E-FINAL-PACKET-HARDENING; AUTHORIZE_FINAL_OPUS_WP10_EVIDENCE_HERMETICITY_REREVIEW (WP10 FINAL_PHASE_B_PASS, closeout b132166).
  childPlanStatus: CONFIRM_REQUIRED   # substantive Macro-E architecture approved; corrected profile/matrix semantic digests prepared but NOT yet Human-bound; Build NOT authorized
  programStatus: APPROVED_IDLE   # WP04–WP12 COMPLETE; Macro-E substantive decisions consumed; profile digest rebind required before Build; next gate APPROVE_CORRECTED_HTR_PROFILE_DIGEST_AND_MACRO_E_BUILD
  governanceStateAuthority: GIT_HEAD
  governanceReconciledFromHead: 04386b59351adc5decaf65b64267f008c6cd5e94   # reconciled from HEAD 04386b5 (Macro-E approval); this digest-reconciliation commit advances HEAD by one (43 ahead)
  latestValidatedProductionCodeSha: 993fdaba0ffd5f66837bea1c7272507183efa973
  planningTerminalState: PROFILE_DIGEST_RECONCILIATION_COMPLETE   # governance-only status fields removed from runtime profile/matrix JSON; corrected digests computed; awaiting Human APPROVE_CORRECTED_HTR_PROFILE_DIGEST_AND_MACRO_E_BUILD
  ...
  composerTerminalState: PROFILE_DIGEST_RECONCILIATION_COMPLETE
  branch: dee-415-ai-trader-historical-test-readiness
  branchCreated: true
  buildStarted: true
  currentWorkPackage: HTR-WP13
  activeChildPlan: .cursor/plans/dee-415-htr-wp13-wp16-intelligence-rolling.plan.md   # WP13–WP16 intelligence rolling controller (ACTIVE_MACRO_E_APPROVED_PENDING_PROFILE_DIGEST_REBIND); WP04–WP12 runtime-substrate controller retired
  intelligenceTranche:
    controller: .cursor/plans/dee-415-htr-wp13-wp16-intelligence-rolling.plan.md
    status: APPROVED_PENDING_PROFILE_DIGEST_REBIND
    ...
    activeMacroStatus: CONFIRM_REQUIRED   # substantive architecture approved 2026-07-15; profile/matrix digest rebind pending Human binding before Build
    ...
    d3: SUBSTANTIVE_CONTRACT_HUMAN_APPROVED_DIGEST_REBIND_REQUIRED   # CLARIFY-HTR-D3 consumed 2026-07-15; governance-only status fields removed from runtime JSON; corrected canonical digest a9666258… awaits Human rebind
    d4: HUMAN_DECISION_REQUIRED_BEFORE_HTR_WP14
    d20: HUMAN_DECISION_REQUIRED_BEFORE_HTR_WP16
    htrHistoricalIntelligenceProfileV1: SUBSTANTIVE_CONTRACT_HUMAN_APPROVED_DIGEST_REBIND_REQUIRED
    profileDigestCanonical: a9666258e046e934b2156e0dc3ad5da02eb9b7e69d994e3505a3af5b62cd8bf0   # corrected semantic object (governance fields removed); NOT yet Human-bound
    profileDigestInsertionOrder: 1e57b18197395b92a7eebeb534ac9d20c10bdd7b31694a45cc248569d6beeec6
    profileDigestRawFileSha256: 84ad49c7ae67bbc56ccfb073a987fd65d468e33e98b827f28cd42199b8f2f78d
    profileDigestSupersededApproved: 9a1ed67e39c2a5e23bb83459f4f1fb52b10c88a49ebbf773b5285c90b82f706c   # SUPERSEDED_BY_SEMANTIC_GOVERNANCE_FIELD_REMOVAL (never matched staging bytes under declared canonicalizer)
    ...
    timeframeEvidenceLaneAuthorityMatrixV1: SUBSTANTIVE_CONTRACT_HUMAN_APPROVED_DIGEST_REBIND_REQUIRED   # matrix status field removed; corrected canonical digest 231712e… awaits Human rebind
    matrixDigestCanonical: 231712e2916370e07def8546f70ed8434e7794e498842390c301e9a7a16baffb   # corrected semantic object (status removed); NOT yet Human-bound
    ...
    matrixDigestSupersededApproved: c91868142e8b5ed0b5db533e1811fe4a733290a204efab52f02ed592c1c01b08   # SUPERSEDED_BY_GOVERNANCE_FIELD_REMOVAL (never matched staging bytes under declared canonicalizer)
    ...
    macroEReadiness: READY_FOR_HUMAN_PROFILE_DIGEST_REBIND_AND_BUILD_AUTHORIZATION   # substantive Macro-E decisions consumed; corrected profile/matrix digests prepared; Human digest rebind + Build authorization remain separate gates; BUILD_AUTHORIZED NO
  ...
  nextHumanGate: APPROVE_CORRECTED_HTR_PROFILE_DIGEST_AND_MACRO_E_BUILD
  nextAction: "HUMAN_REBIND_PROFILE_DIGEST_AND_AUTHORIZE_MACRO_E_BUILD (Human gate APPROVE_CORRECTED_HTR_PROFILE_DIGEST_AND_MACRO_E_BUILD). Substantive Macro-E decisions consumed 2026-07-15 at 04386b5 (D-2, D-3 profile contract, matrix contract, CONFIRM-HTR-MACRO-E-MIGRATION, RATIFY strategy-discovery foundation, APPROVE-HTR-INTELLIGENCE-MACRO-E). Digest-integrity reconciliation 2026-07-15: governance-only fields removed from runtime profile/matrix JSON (profile: status, strategyConsumerPolicy.status, decisionBasis, strategyConsumerPolicy.ownedBy; matrix: status). Corrected profile canonical digest a9666258… (raw 84ad49c7…); corrected matrix canonical digest 231712e… (raw 4aed27c0…). Prior approved digests 9a1ed67e…/c9186814… SUPERSEDED (never matched staging bytes under declared canonicalizer; controller §5 displayed truncated object not verbatim staging). Six substantive decisions remain consumed; corrected digests NOT yet Human-bound. Build NOT authorized. No WP13 code/SQL/PR."
provenance:
...
> **Macro-E readiness is `READY_FOR_HUMAN_PROFILE_DIGEST_REBIND_AND_BUILD_AUTHORIZATION`** — substantive Macro-E architecture was Human-approved 2026-07-15 (`APPROVE_FINAL_D2_D3_MATRIX_MIGRATION_STRATEGY_FOUNDATION_AND_MACRO_E_PACKET` at `04386b5`). Digest-integrity reconciliation 2026-07-15 removed governance-only fields from runtime profile/matrix JSON; corrected profile canonical digest `a9666258…` and matrix canonical digest `231712e…` await narrow Human rebind (`APPROVE_CORRECTED_HTR_PROFILE_DIGEST_AND_MACRO_E_BUILD`). Build NOT authorized. The WP10 evidence-hermeticity correction is **CLOSED `FINAL_PHASE_B_PASS`** (2026-07-14, closeout commit `b132166`; final Opus Phase-B PASS at `521ddd1`). Root cause (historical): a read-only forensic proved the original `pnpm test --run` overwrote the tracked sealed WP10 evidence bundle (`WP10_SEALED_EVIDENCE_MUTATION` + `WP10_TEST_HARNESS_NOT_HERMETIC`). The bounded correction chain — `017fcbe` (FAIL symlink/case-alias), `f2413b2` (FAIL output-directory-substitution TOCTOU), `521ddd1` (PASS cwd/inode-bound child publication + completion contract) — makes the normal suite read/assert-only, adds static path guards + a TOCTOU-safe writer, and accepts a **separate** post-Macro-D compatibility candidate at `replay-runs/RI-P7/htr-wp10-determinism-nolookahead-post-macro-d/` (compatibility artifact `3d3f7cf3…`). The original WP10 seal (`replay-runs/RI-P7/htr-wp10-determinism-nolookahead/`, artifact `fa5def37…`) remains **byte-identical** and was **not** replaced. `LATEST_VALIDATED_PRODUCTION_CODE_SHA` remains `993fdab`; the correction chain touches only the WP10 evidence writer/harness/tests (no production runtime, strategy, risk, market-data, clock/id, or no-lookahead change).
...
- **D-3 — SUBSTANTIVE_CONTRACT_HUMAN_APPROVED; DIGEST_REBIND_REQUIRED.** `CLARIFY-HTR-D3-HISTORICAL-PROFILE` consumed 2026-07-15. Runtime semantic contract approved; governance-only fields (`status`, `strategyConsumerPolicy.status`, `decisionBasis`, `strategyConsumerPolicy.ownedBy`) removed from the staging JSON 2026-07-15. Corrected canonical digest `a9666258e046e934b2156e0dc3ad5da02eb9b7e69d994e3505a3af5b62cd8bf0` awaits narrow Human rebind (`APPROVE_CORRECTED_HTR_PROFILE_DIGEST_AND_MACRO_E_BUILD`). Prior digest `9a1ed67e…` is **SUPERSEDED_BY_SEMANTIC_GOVERNANCE_FIELD_REMOVAL** (never matched staging bytes under the declared canonicalizer).
...
### `HTR_HISTORICAL_INTELLIGENCE_PROFILE_V1` (SUBSTANTIVE_CONTRACT_HUMAN_APPROVED · DIGEST_REBIND_REQUIRED)

Explicit, versioned historical run profile (staging: `.cursor/plans/dee-415-htr-wp13-wp16-staging/htr-historical-intelligence-profile-v1.json`) that activates the record-level MI-core chain **only** through an explicit seam (`runBacktest historicalProfile → runEvaluationCycle`), **never** as a global default. Contains **no governance/planning status fields** — approval state lives only in canonical/controller governance YAML. Pins venueScope `HTX_ONLY`, marketType `SPOT`, symbols `[BTCUSDT, ETHUSDT]`, baseInterval `1m`, derivedIntervals `[15m,1h,4h,1d]` (`CLOSED_BARS_ONLY`); enabled intelligence stages; `strategyConsumerPolicy` with enabled LSR+MR and research-only TM (`EVIDENCE_ONLY_NOT_TRADE_ELIGIBLE`); `historicalEvidenceCapability: PRICE_ONLY_GROUNDED_EVIDENCE_PROFILE`; matrix digest pin in `providerEvidenceLanePolicy.matrixDigestCanonical`. **Corrected canonical digest:** `a9666258e046e934b2156e0dc3ad5da02eb9b7e69d994e3505a3af5b62cd8bf0` (insertion-order `1e57b181…`; raw-file `84ad49c7…`). **Prior approved digest** `9a1ed67e…` **SUPERSEDED** (digest-integrity defect: controller §5 displayed a truncated object, not verbatim staging bytes). **Substantive runtime semantics unchanged** — only governance-only fields removed.

### `TIMEFRAME_EVIDENCE_LANE_AUTHORITY_MATRIX_V1` (SUBSTANTIVE_CONTRACT_HUMAN_APPROVED · DIGEST_REBIND_REQUIRED)

Exact machine-readable matrix (staging: `.cursor/plans/dee-415-htr-wp13-wp16-staging/timeframe-evidence-lane-authority-matrix-v1.json`) over **16 lanes** (1 HTX spot 1m price + 15 UNAVAILABLE sidecar lanes). Contains **no governance status fields**. **Corrected canonical digest:** `231712e2916370e07def8546f70ed8434e7794e498842390c301e9a7a16baffb` (insertion-order `2e558f18…`; raw-file `4aed27c0…`). **Prior approved digest** `c9186814…` **SUPERSEDED** (governance `status` field removed; prior digest never matched staging bytes under declared canonicalizer). FHV scope: `PRICE_ONLY_GROUNDED_EVIDENCE`; `MULTI_SOURCE_HISTORICAL_VALIDATION: NOT_PERFORMED`.
...
The **WP04–WP12 rolling runtime/data-truth tranche is COMPLETE** and the **WP13–WP16 intelligence tranche is `APPROVED_PENDING_PROFILE_DIGEST_REBIND`** (substantive Macro-E packet Human-approved 2026-07-15 at `04386b5`; digest-integrity reconciliation removes governance-only fields from runtime profile/matrix JSON — see §"WP13–WP16 intelligence tranche"). The program is `APPROVED_IDLE` at the **`APPROVE_CORRECTED_HTR_PROFILE_DIGEST_AND_MACRO_E_BUILD`** Human gate. Topology: **HTR-MACRO-E [WP13]** (exact packet, substantive contract approved; profile/matrix digest rebind required) / **HTR-MACRO-F [WP14,WP15]** (architecture-only) / **HTR-MACRO-G [WP16]** (architecture-only), in `.cursor/plans/dee-415-htr-wp13-wp16-intelligence-rolling.plan.md`. D-1 is **ALREADY_RESOLVED** (record-level chain, no mature autonomous engines); D-3 is **SUBSTANTIVE_CONTRACT_HUMAN_APPROVED · DIGEST_REBIND_REQUIRED** (`HTR_HISTORICAL_INTELLIGENCE_PROFILE_V1`, corrected digest `a9666258…`; prior `9a1ed67e…` SUPERSEDED; `historicalEvidenceCapability = PRICE_ONLY_GROUNDED_EVIDENCE_PROFILE`); D-2 is **HUMAN_APPROVED** (consumed 2026-07-15: HTX SPOT BTCUSDT+ETHUSDT; base 1m + derived 15m/1h/4h/1d; enabled `liquidity_sweep_reversal_v0`+`mean_reversion_v0`; research-only `trend_momentum_v0` = `EVIDENCE_ONLY_NOT_TRADE_ELIGIBLE`; enablement is **not** SVG approval and **not** an edge verdict; version-pin+lifecycle owned by WP16); `TIMEFRAME_EVIDENCE_LANE_AUTHORITY_MATRIX_V1` is **SUBSTANTIVE_CONTRACT_HUMAN_APPROVED · DIGEST_REBIND_REQUIRED** (16 lanes, corrected digest `231712e…`; prior `c9186814…` SUPERSEDED; `FHV_SCOPE_LIMITATION=PRICE_ONLY_GROUNDED_EVIDENCE` + `MULTI_SOURCE_HISTORICAL_VALIDATION=NOT_PERFORMED`). WP13 `MIGRATION_DECISION` is **`CONFIRMED_WITH_ATOMIC_FAIL_CLOSED_IDEMPOTENCY`** (3 append-only Postgres tables — `trader_intelligence_cycle_envelope`/`hypothesis_record`/`conviction_record`, service-role RLS; **atomic cycle bundle** `HTR_WP13_ATOMIC_INTELLIGENCE_CYCLE_BUNDLE_V1` single Postgres transaction + partial-commit PROHIBITED; **fail-closed idempotency** — load-and-compare deterministic id + content digest, `HTR_WP13_IDEMPOTENCY_CONFLICT` on any mismatch, `ON CONFLICT DO NOTHING` = **PROHIBITED_SUPERSEDED_DESIGN**; **conviction cardinality Model B** `MODEL_B_ONE_CYCLE_LEVEL_ACTIVE_CONVICTION` `UNIQUE(org,run,cycle,symbol)` + NULLABLE `active_hypothesis_record_id` + `conviction_scope` ∈ {`ACTIVE_HYPOTHESIS`,`NONE`}, `AGGREGATE` = `DEFERRED_NOT_IN_SCHEMA_V1`; hypothesis `UNIQUE(org,run,cycle,symbol,hypothesis_type)`; **`SQLITE_ADAPTER: NOT_REQUIRED`**; DUP-14 link; migrations 0076–0081; Forecast/Decision/whyNotCash DEFERRED to WP14; no SQL created) and `STRATEGY_DISCOVERY_TO_OPERATOR_PROPOSAL_CONTRACT_V1` is **HUMAN_RATIFIED** (foundation only). **`MACRO_E_READINESS: READY_FOR_HUMAN_PROFILE_DIGEST_REBIND_AND_BUILD_AUTHORIZATION`** — corrected profile/matrix digests prepared but NOT yet Human-bound; Build NOT authorized. WP10 evidence-hermeticity correction is **CLOSED `FINAL_PHASE_B_PASS`** (2026-07-14, closeout `b132166`). WP13 (intelligence-chain activation, historical run profile) remains **blocked from Build** until corrected digests are Human-bound and Build is separately authorized. No WP13 code, no SQL and no PR were created in this session; the single final PR remains gated on all 23 WPs. Completed WORK commits: HTR-WP01 `6600708`; WP02 `7ec02dd`; WP03 `35283ed`; WP04 `b3abe7b`; WP05 `f90faa9`; WP06 `24eb7f9`; WP07 `10f2500`; WP08 `0c4b8c3`; WP09 `46820ac`; WP10 `befa6c1`; WP11 `f6cefb0`; WP12 `993fdab`. This heading also satisfies the canonical-plan validator's `## WP-*` requirement.
