...
Migration 0217 adds only two partial lookup indexes on immutable observations, using the exact existing balance-complete and first-success predicates; no ledger rewrite, grants, RLS or financial semantics change. The local 8,000-observation EXPLAIN regression proves the first-success lookup stops scanning all partial records. Local migration and 19 focused snapshot/entity/hook/index tests pass. The independently validated additive indexes may be staged before the code merge under the user-delegated operational repair gate: create concurrently to avoid blocking observation writes, verify both definitions/validity, then record the exact canonical Drizzle migration hash only after the compatible code is deployed. Existing execution-host checkouts must also recognize the additive migration before that journal step; otherwise leave the indexes staged and the journal pending. Full exact-head CI remains mandatory for the code merge/deployment; verify index validity and the actual browser read latency. Rollback is code rollback with additive indexes retained, not destructive ledger changes.
...
CI correction: the full migration guard rejected 0217 because explicit FHV compatibility admission was missing. Add only 0217 identity to the existing allowlist, retaining required prefix 0000–0207, exact hashes and rejection of unknown successors. The original failing CI evidence is retained. No FHV policy or eligibility is relaxed.

## Production session correction

Long-running production acceptance exposed Auth token timeouts and a lost refresh token. The existing RSC session client intentionally discards cookie updates and no middleware persisted refreshes, so a server refresh could leave the browser holding an obsolete token. Add a narrowly scoped middleware refresh only for the Trader admin page and console API paths when an existing Supabase session cookie is present. The actual SDK verifies the user; refreshed cookies follow the existing cookie-domain policy and are forwarded to the same server render. Protected handlers still independently verify identity and permissions. No identity header, new session, role, credential or grant is synthesized. Legacy authentication and other hosts/routes remain unchanged. Provider failures return 503 with an explicit reason and preserve browser cookies rather than mislabeling the failure as revoked access.

The previously failing middleware assertions now pass. An actual SSR SDK regression with a synthetic expired JWT and stubbed Auth transport proves refresh-token rotation, a verified user request, browser Set-Cookie and server cookie propagation. Existing auth/revoke tests pass. This minimal critical-path correction uses the user's explicit operational Human-gate delegation; no independent reviewer or financial policy ratification is claimed. Repeat full exact-head CI and production acceptance after the change.
