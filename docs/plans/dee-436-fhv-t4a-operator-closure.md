...
|----|-------------|-----------------|-------------|--------------------------|---------------|---------|--------------|---------------|---------------|---------------|
| R01 | Release SHA and tag identity | FHV-RELEASE-IDENTITY-CONTRACT | `scripts/ops/validate-fhv-release-identity.sh` | `scripts/ops/validate-fhv-release-identity.sh --repo-path "$CHECKOUT_ROOT" --target-sha "$EXECUTION_SERVER_TARGET_SHA"` | text PASS/FAIL | PASS + exact SHA/tag | nonzero | existing release-identity tests | wrong SHA | preflight logs |
...
| R21 | Disconnect/reconnect continuity | rehearsal contract | continuity digests | packet captures + ceremony | path→sha256 maps | before==after required keys | 1 | continuity helper | changed digest | continuity JSON pair |
| R22 | Observer restart continuity | rehearsal contract | same | continuity digests include status/observer restart proof | equal digests | 1 | continuity | changed | continuity JSON |
| R23 | Total five-minute budget | launcher maxRuntime | `fhv-t4-campaign-runtime.v1.json` | `trader:fhv:t4:verify-final` | shared elapsed≤300000 | one monotonic window | 1 | budget exceeded negative | 300001ms | campaign runtime proof |
