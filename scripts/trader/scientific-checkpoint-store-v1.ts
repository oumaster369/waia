...
function fail(reason: string): never {
  throw new Error(`SCIENTIFIC_CHECKPOINT_REFUSED:${reason}`);
}
...
function inputDigest(value: unknown): string {
  ...
  const visit = (v: unknown): void => {
    ...
    if (typeof v === "number") {
      ...
      bytes.writeDoubleBE(v);
      h.update("number:").update(bytes);
      return;
    }
    if (Buffer.isBuffer(v)) {
      h.update(`buffer:${v.length}:`).update(v);
      return;
    }
    if (typeof v === "string") {
      const bytes = Buffer.from(v);
      h.update(`string:${bytes.length}:`).update(bytes);
      return;
    }
...
/** Existing-store hydrate ONLY. No mkdir, seal-key creation, builder, or write. */
export function readPreservedScientificPackageNoBuildV1(
  root: string,
  packageKey: string,
): PredictivePackageV1 {
  if (process.release?.name !== "node" || process.env.WAIA_TRADER_CLI !== "1") fail("NODE_CLI");
  if (!isAbsolute(root) || resolve(root) === "/" || !HEX.test(packageKey)) fail("CONFIG");
  ...
  const target = join(root, packageKey);
  if (!existsSync(target)) fail("PACKAGE_MISSING");
  privatePath(target, true);
  const envelope = JSON.parse(
    readBounded(join(target, "seal.json"), MAX_METADATA_BYTES).toString("utf8"),
  ...
  return hydratePredictivePackageV1(m, chunks(), {
    ...expected,
...
export function createScientificCheckpointStoreV1(
  ...
  if (secret.length !== 32) fail("SEAL_KEY");
  const sign = (body: string) => createHmac("sha256", secret).update(body).digest("hex");
  const identity = (stage: string, input: unknown) =>
    inputDigest({
      format: FORMAT,
      releaseSha,
  ...
  return {
    package(input: PackageBuildInputV1, build: () => PredictivePackageV1): PredictivePackageV1 {
      ...
      const key = identity("predictive-package", input),
        target = join(root, key);
      if (existsSync(target)) {
        const seal = readSeal(target, key, "package"),
          expected = seal.packageIdentity;
        if (
          !expected ||
      ...
      const pkg = build();
      const partial = mkdtempSync(join(root, `${key}.partial-`));
      const stream = streamEncodePredictivePackageV1(pkg);
      let ordinal = 0;
      for (;;) {
        ...
        if (step.done) {
          ...
          publish(partial, target, {
            ...
            kind: "package",
            key,
            payloadDigest: step.value.manifestDigestHex,
            manifestFileDigest: hash(manifestBytes),
            packageIdentity: {
              organizationId: step.value.organizationId,
              generationDigestHex: step.value.generationDigestHex,
              contentDigestHex: step.value.contentDigestHex,
            },
          });
          return pkg;
        }
    ...
    evidence<T>(stage: string, input: unknown, build: () => T): T {
      ...
      const key = identity(stage, input),
        target = join(root, key);
    ...
    async evidenceAsync<T>(stage: string, input: unknown, build: () => Promise<T>): Promise<T> {
      ...
      return existsSync(target)
        ? loadEvidence<T>(target, key)
        : saveEvidence(target, key, await build());
    },
  };
}
