...
const { INTERNAL_createValidationBootstrapRangeEvaluatorV1 } = tsxRequire(
  "../../lib/trader/research/benchmark/validation-bootstrap-v1.ts", import.meta.url,
);
const evaluateRange = INTERNAL_createValidationBootstrapRangeEvaluatorV1({
...
parentPort.on("message", ({ start, endExclusive }) => {
  try {
    parentPort.postMessage({ ok: true,
      result: evaluateRange(start, endExclusive) });
  } catch (error) {
