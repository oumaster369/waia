...
import {
  ...
  type INTERNAL_validationBootstrapOrdinalRangeV1,
...
const RANGE_SIZE = VALIDATION_BOOTSTRAP_RANGE_SIZE;
  ...
  try {
    return await new Promise<ValidationBootstrapNullCenteredResultV1>((resolve, reject) => {
      ...
      const assign = (worker: Worker) => {
        ...
        const range = {
          start: nextStart,
          endExclusive: Math.min(VALIDATION_BOOTSTRAP_B, nextStart + RANGE_SIZE),
        };
      ...
      try {
        ...
        for (let i = 0; i < workerCount; i++) {
          ...
          worker.on("exit", (code) => {
            if (!settled) fail(new Error(`VALIDATION_BOOTSTRAP_WORKER_EXIT:${code}`));
          });
          worker.on("message", (message: unknown) => {
            ...
            try {
              ...
              if (
                ...
                result.n !== n ||
                !Object.is(result.dBar, dBar) ||
                !Object.is(result.tObs, tObs) ||
                !Object.is(result.centeredMean, centeredMean) ||
                !Number.isSafeInteger(result.extremeCount) ||
                result.extremeCount < 0 ||
                result.extremeCount > result.endExclusive - result.start
              ) {
              ...
              if (completed === VALIDATION_BOOTSTRAP_B) {
                if (
                  assignments.size !== 0 ||
                  nextStart !== VALIDATION_BOOTSTRAP_B ||
