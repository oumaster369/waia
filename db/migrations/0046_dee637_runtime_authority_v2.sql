...
--> statement-breakpoint
CREATE TRIGGER `trader_runtime_control_leases_v2_update_guard` BEFORE UPDATE ON `trader_runtime_control_leases_v2` BEGIN SELECT RAISE(ABORT,'RUNTIME_CONTROL_LEASE_V2_APPEND_ONLY'); END;
--> statement-breakpoint
CREATE TRIGGER `trader_runtime_control_leases_v2_delete_guard` BEFORE DELETE ON `trader_runtime_control_leases_v2` BEGIN SELECT RAISE(ABORT,'RUNTIME_CONTROL_LEASE_V2_APPEND_ONLY'); END;
