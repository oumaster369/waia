...
| Postgres `0174_waia_admin_hr` / `0175_*_rls` | `waia_admin_module_grants`, `hr_team_applications`, `hr_application_events` | DEE-747: role-scoped shared WAIA Admin Finance/HR grants and public team-application funnel with immutable intake, append-only history, one-way revocation, no hard delete and deny-by-default browser RLS. These grants do not confer AI-TRADER Admin authority. |
