...
export function AuthBlock() {
  ...
  const primaryCtaLabel = isLoading ? "…" : mode === "createTwin" ? "Create Twin" : "Sign in";
  ...
  return (
    <section
      ...
      aria-busy={isLoading}
      className="mx-auto flex w-full max-w-md flex-col gap-4 rounded-[var(--waia-radius-ceremonial-lg)] border border-[color-mix(in_oklch,var(--waia-color-rim)_58%,transparent)] bg-[color-mix(in_oklch,var(--waia-color-auth-field-veil)_68%,transparent)] p-6 text-card-foreground shadow-none backdrop-blur-[var(--waia-blur-veil-lg)] backdrop-saturate-[108%] sm:p-8"
    >
      ...
      {confirmationNotice && mode === "signIn" ? (
        <div
          ...
          role="status"
          className="rounded-lg border border-[color-mix(in_oklch,var(--waia-color-rim)_72%,transparent)] bg-[color-mix(in_oklch,var(--waia-color-fg-primary)_7%,transparent)] px-3 py-2 text-muted-foreground text-sm leading-snug"
        >
          Account created. Check your inbox to confirm before signing in.
        </div>
      ) : null}

      <div className="flex justify-center rounded-lg border border-[color-mix(in_oklch,var(--waia-color-rim)_62%,transparent)] bg-[color-mix(in_oklch,var(--waia-color-fg-primary)_5%,transparent)] p-1 text-sm font-medium">
        <button
          type="button"
          ...
          className={cn(
            ...
            mode === "createTwin"
              ? "bg-[color-mix(in_oklch,var(--waia-color-fg-primary)_12%,transparent)] text-foreground shadow-[inset_0_1px_0_0_oklch(1_0_0/0.07)]"
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3" noValidate>
        {mode === "createTwin" ? (
          <label className="flex flex-col gap-1.5 text-sm">
            ...
            <Input
              data-testid="landing-auth-display-name"
              name="displayName"
              type="text"
              autoComplete="name"
              value={displayName}
            ...
            ? (
                <>
                  <div className="flex items-center gap-3 text-xs uppercase tracking-wide text-muted-foreground">
                    <span aria-hidden="true" className="h-px flex-1 bg-[color-mix(in_oklch,var(--waia-color-divider)_75%,transparent)]" />
