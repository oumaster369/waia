...
import type { TreasuryBudgetDto } from "@/lib/treasury-admin/types";

function BudgetsInner() {
  const { organizationId } = useFinanceOrg();
  const [rows, setRows] = React.useState<TreasuryBudgetDto[]>([]);
  ...
  const load = React.useCallback(async () => {
    ...
    setLoading(true);
    const result = await treasuryGet<{ budgets: TreasuryBudgetDto[] }>(
