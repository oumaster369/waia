...
function CategoryRow({
  ...
  async function apply() {
    ...
    if (pending === "save" && (!parsed.ok || !name.trim() || !groupName.trim())) {
      setPending(null);
      setError(
        !parsed.ok ? parsed.message : "Name, group, and a non-negative monthly limit are required.",
      );
      return;
    }
    if (!reason.trim()) {
    ...
    <WaiaSurface variant="raised" className="space-y-3 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <dl className="grid grid-cols-3 gap-x-5 gap-y-1 text-right text-xs">
          ...
          <dd>
            <BudgetAmount micros={row.budgetMicros} currency={row.currency} />
          </dd>
          <dd>
            <BudgetAmount micros={row.spentMicros} currency={row.currency} />
          </dd>
          <dd>
            <BudgetAmount micros={row.remainingMicros} currency={row.currency} />
          </dd>
        </dl>
      </div>
      <MoreDetails summary="Edit category">
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <FormField label="Group" htmlFor={`category-group-${row.categoryId}`}>
            <Input
              id={`category-group-${row.categoryId}`}
          ...
          <FormField
            label="Monthly limit"
            htmlFor={`category-monthly-${row.categoryId}`}
            help="Enter a non-negative expense limit. Use zero to reserve a category without current cost."
          >
            <Input
              id={`category-monthly-${row.categoryId}`}
              inputMode="decimal"
              value={monthly}
        ...
        <FormField label="Description" htmlFor={`category-description-${row.categoryId}`}>
          <Textarea
            id={`category-description-${row.categoryId}`}
            value={description}
      ...
      <ConfirmDialog
        ...
        impact={
          pending === "archive"
            ? "Historical transactions keep this category, but it leaves active selectors."
            : "The new limit and group become the effective monthly budget input. Historical months remain intact."
        }
...
export function CategoryBudgetPanel() {
  ...
  const query = React.useCallback((): Promise<
    TreasuryApiResult<{ month: TreasuryCategoryBudgetMonthDto }>
  > => {
  ...
  const summary = useTreasuryQuery(
    Boolean(organizationId),
    `category-budgets:${organizationId ?? ""}`,
    query,
  );
  ...
  async function create() {
    ...
    if (!parsed.ok || !name.trim() || !groupName.trim() || !currency.trim()) {
      ...
      setCommandError(
        !parsed.ok
          ? parsed.message
          : "Name, group, currency, and a non-negative monthly limit are required.",
      );
      return;
    }
    setBusy(true);
  ...
  if (displayError)
    return (
      <UnavailableState code={displayError.code} message={displayError.message} onRetry={reload} />
    );
  ...
  return (
    <div className="space-y-4" data-testid="finance-category-budgets">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-muted-foreground text-xs tracking-wide uppercase">Current month</p>
          <h3 className="text-lg font-medium">{month?.month ?? "—"}</h3>
        </div>
        <p className="text-muted-foreground max-w-md text-xs">
          Spend and remaining are derived from verified transactions by the server.
        </p>
      </div>
            ...
            {[
              ...
              <div key={label}>
                ...
                <dd className="mt-1 text-lg">
                  <BudgetAmount micros={value} currency={total.currency} />
                </dd>
              </div>
            ))}
          </dl>
              ...
              {month.groups.map((group) => (
                <tr key={`${group.groupName}:${group.currency}`} className="border-b last:border-0">
                  ...
                  <td className="p-3 text-right">
                    <BudgetAmount micros={group.budgetMicros} currency={group.currency} />
                  </td>
                  <td className="p-3 text-right">
                    <BudgetAmount micros={group.spentMicros} currency={group.currency} />
                  </td>
                  <td className="p-3 text-right">
                    <BudgetAmount micros={group.remainingMicros} currency={group.currency} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {!month || month.categories.length === 0 ? (
        ...
        month.categories.map((row) => (
          <CategoryRow
            ...
            row={row}
            detail={categoryDetails.get(row.categoryId)}
            onSaved={reload}
          />
        ))
      )}
      <WaiaSurface variant="raised" className="space-y-3 p-4">
        <div>
          ...
          <p className="text-muted-foreground text-xs">
            The unique category code is generated automatically.
          </p>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <FormField label="Name" htmlFor="new-category-name">
            <Input
          ...
          <FormField
            label="Group"
            htmlFor="new-category-group"
            help="Choose a suggestion or type a new group."
          >
            <Input
          ...
          <FormField
            label="Monthly limit"
            htmlFor="new-category-monthly"
            help="Enter a non-negative expense limit without a minus sign. Zero reserves a category for future cost."
          >
            <Input
              id="new-category-monthly"
              inputMode="decimal"
              value={monthly}
              onChange={(event) => setMonthly(event.target.value)}
            />
          </FormField>
          <FormField label="Currency" htmlFor="new-category-currency">
            <Input
              id="new-category-currency"
              value={currency}
              onChange={(event) => setCurrency(event.target.value.toUpperCase())}
            />
          </FormField>
        </div>
        <FormField label="Description (optional)" htmlFor="new-category-description">
          <Textarea
