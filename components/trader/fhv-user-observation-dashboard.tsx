...
function FhvUserObservationDashboardForRun({ runId }: { runId: string }): React.ReactNode {
  ...
  return <section className="space-y-5" data-testid="fhv-user-streaming-dashboard">
    ...
    <div className="grid gap-4 lg:grid-cols-3"><Feed title="Open positions" items={view.positions} empty={num(view.balance.openPositionsCount) ? `${text(view.balance.openPositionsCount)} open position(s); row details were not published by this producer.` : "No open simulated positions."} /><Feed title="Recent trades" items={view.trades} empty={num(view.balance.fillsCount) ? `${text(view.balance.fillsCount)} simulated fill(s); row details were not published by this producer.` : "No simulated fills yet."} /><Feed title="Recent orders" items={view.orders} empty={num(view.balance.ordersCount) ? `${text(view.balance.ordersCount)} simulated order(s); row details were not published by this producer.` : "No simulated orders yet."} /></div>
