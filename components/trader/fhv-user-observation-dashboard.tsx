...
export type UserStreamView = { observedAt: string | null; progress: Record<string, unknown>; balance: Record<string, unknown>; positions: readonly Summary[]; trades: readonly Summary[]; orders: readonly Summary[]; tradeCounts: Readonly<{ fills: number | null; orders: number | null }>; decision: Record<string, unknown>; checkpoint: Record<string, unknown>; risk: Record<string, unknown>; gate: Record<string, unknown> };
export const EMPTY_USER_STREAM_VIEW: UserStreamView = { observedAt: null, progress: {}, balance: {}, positions: [], trades: [], orders: [], tradeCounts: { fills: null, orders: null }, decision: {}, checkpoint: {}, risk: {}, gate: {} };
...
export function reduceFhvUserStreamEvent(state: UserStreamView, event: RealtimeEvent): UserStreamView {
  ...
  switch (event.kind) {
    ...
    case "trade.snapshot": return { ...base, trades: rows(event.payload.recentFills), orders: rows(event.payload.recentOrders), tradeCounts: { fills: num(event.payload.fillsCount), orders: num(event.payload.ordersCount) } };
...
function FhvUserObservationDashboardForRun({ runId }: { runId: string }): React.ReactNode {
  ...
  const terminalCompleted = view.progress.terminalState === "COMPLETED";
  const completion = terminalCompleted ? 100 : (num(view.progress.completionPct) ?? 0);
  ...
  return <section className="space-y-5" data-testid="fhv-user-streaming-dashboard">
    ...
    <WaiaSurface variant="elevated" className="space-y-4 p-5"><div className="flex flex-wrap justify-between gap-3 text-sm"><span>{text(view.progress.phase)} · {text(view.progress.currentSymbol)} · {text(view.progress.partition)}</span><span className="text-muted-foreground tabular-nums">{text(view.progress.barsProcessed)} economic replay bars</span></div><div className="bg-muted h-3 overflow-hidden rounded-full"><div className="h-full rounded-full bg-emerald-500 transition-[width] duration-500" style={{ width: `${Math.max(0, Math.min(100, completion))}%` }} /></div><div className="flex justify-between text-xs"><span>{completion.toFixed(1)}% complete</span><span className="text-muted-foreground">{terminalCompleted ? `Completed · ${text(view.progress.barsTotal)} qualified corpus bars` : `ETA ${text(view.progress.etaUtc)}`}</span></div></WaiaSurface>
    ...
    <div className="grid gap-4 lg:grid-cols-3"><Feed title="Open positions" items={view.positions} empty={num(view.balance.openPositionsCount) ? `${text(view.balance.openPositionsCount)} open position(s); row details were not published by this producer.` : "No open simulated positions."} /><Feed title="Recent trades" items={view.trades} empty={view.tradeCounts.fills ? `${view.tradeCounts.fills} simulated fill(s); row details were not published by this producer.` : "No simulated fills yet."} /><Feed title="Recent orders" items={view.orders} empty={view.tradeCounts.orders ? `${view.tradeCounts.orders} simulated order(s); row details were not published by this producer.` : "No simulated orders yet."} /></div>
